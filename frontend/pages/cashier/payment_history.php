...
<style>
    ...
    .payment-card .payment-header .receipt-number {
        font-weight: 700;
        font-size: 1rem;
        color: var(--text-primary);
    ...
    .payment-card .payment-header .payment-amount {
        font-size: 1.2rem;
        font-weight: 700;
        color: #059669;
    }
    ...
    .payment-card .payment-body .info-item .label {
        font-size: 0.6rem;
        color: var(--text-secondary);
    ...
    .payment-card .payment-body .info-item .value {
        font-size: 0.9rem;
        font-weight: 500;
        color: var(--text-primary);
    }
    ...
    .payment-card .payment-footer .payment-method {
        font-size: 0.7rem;
        font-weight: 600;
        padding: 2px 12px;
        border-radius: 20px;
    ...
    .payment-card .payment-footer .payment-method.cash {
        background: #E8F0FE;
        color: #0B5ED7;
    }
    
    .payment-card .payment-footer .payment-method.m-pesa {
        background: #D1FAE5;
        color: #059669;
    }
    
    .payment-card .payment-footer .payment-method.card {
        background: #F1F5F9;
        color: #475569;
    }
    
    .payment-card .payment-footer .payment-method.airtel_money {
        background: #FEF3C7;
        color: #D97706;
    }
    
    .payment-card .payment-footer .payment-method.tigo_pesa {
        background: #F3E8FF;
        color: #7C3AED;
    }
    
    .payment-card .payment-footer .payment-method.halopesa {
        background: #FCE4EC;
        color: #DB2777;
    }
    
    .payment-card .payment-footer .payment-method.insurance {
        background: #E0F2FE;
        color: #0284C7;
    }
    
    .payment-card .payment-footer .payment-method.other {
        background: #F1F5F9;
        color: #64748B;
    }
    
    [data-theme="dark"] .payment-card .payment-footer .payment-method.cash {
        background: #1E3A5F;
        color: #6EA8FE;
    }
    
    [data-theme="dark"] .payment-card .payment-footer .payment-method.m-pesa {
        background: #1A3A2A;
        color: #34D399;
    }
    
    [data-theme="dark"] .payment-card .payment-footer .payment-method.card {
        background: #1E293B;
        color: #94A3B8;
    }
    
    [data-theme="dark"] .payment-card .payment-footer .payment-method.airtel_money {
        background: #3D2E0A;
        color: #FBBF24;
    }
    
    [data-theme="dark"] .payment-card .payment-footer .payment-method.tigo_pesa {
        background: #2A1A3A;
        color: #9B4DCA;
    }
    
    [data-theme="dark"] .payment-card .payment-footer .payment-method.halopesa {
        background: #3A1A2A;
        color: #F472B6;
    }
    
    [data-theme="dark"] .payment-card .payment-footer .payment-method.insurance {
        background: #1A2A3A;
        color: #38BDF8;
    }
    
    [data-theme="dark"] .payment-card .payment-footer .payment-method.other {
        background: #1E293B;
        color: #94A3B8;
    }
...
<nav class="top-nav">
    <div class="flex items-center gap-4 flex-1">
        ...
        <div class="search-wrapper">
            ...
            <input type="text" id="searchInput" placeholder="Search by receipt, patient, bill..." 
...
<main class="main-content">
    ...
    <div class="stats-grid animate-fade-in-up">
        ...
        <a href="payment_history.php" class="stat-card purple">
            <div>
    ...
    <div class="summary-stats animate-fade-in-up">
        ...
        <div class="stat-item">
            <p class="stat-number">TSh <?= number_format($total_cash) ?></p>
            <p class="stat-label">Cash</p>
        </div>
        <div class="stat-item">
            <p class="stat-number">TSh <?= number_format($total_mpesa) ?></p>
            <p class="stat-label">M-Pesa</p>
        </div>
        <div class="stat-item">
            <p class="stat-number">TSh <?= number_format($total_card) ?></p>
            <p class="stat-label">Card</p>
        </div>
        <div class="stat-item">
            <p class="stat-number">TSh <?= number_format($total_other) ?></p>
    ...
    <div class="filter-section animate-fade-in-up">
        <form method="GET" action="" class="flex flex-wrap items-center gap-3 w-full">
            ...
            <a href="payment_history.php" class="btn-clear">
                ...
                <div class="payment-card">
                    <div class="payment-header">
                        <div class="receipt-number">
                            ...
                            <span class="text-xs text-gray-400 ml-2">
                                Bill: <?= htmlspecialchars($payment['bill_number']) ?>
                            </span>
                        </div>
                    ...
                    <div class="payment-body">
                        <div class="info-item">
                            ...
                            <div class="value"><?= htmlspecialchars($payment['patient_name'] ?? 'Unknown') ?></div>
                            <div class="text-xs text-gray-400">ID: <?= htmlspecialchars($payment['patient_id'] ?? 'N/A') ?></div>
                        </div>
                        <div class="info-item">
                            <div class="label">Cashier</div>
                            <div class="value"><?= htmlspecialchars($payment['cashier_name'] ?? 'N/A') ?></div>
                        </div>
                        <div class="info-item">
                            <div class="label">Date</div>
                            <div class="value"><?= date('M d, Y', strtotime($payment['received_at'])) ?></div>
                            <div class="text-xs text-gray-400"><?= date('h:i A', strtotime($payment['received_at'])) ?></div>
                        </div>
                        <div class="info-item">
                            <div class="label">Reference</div>
                            <div class="value"><?= htmlspecialchars($payment['reference_number'] ?? 'N/A') ?></div>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <?php if (!empty($payment['notes'])): ?>
                                <span class="text-xs text-gray-400 ml-2">
                                    ...
                                    <?= htmlspecialchars(substr($payment['notes'], 0, 50)) ?><?= strlen($payment['notes'] ?? '') > 50 ? '...' : '' ?>
                                </span>
                            <?php endif; ?>
                        </div>
                        <div class="flex gap-2">
                            <a href="view_bill.php?id=<?= $payment['bill_number'] ?>" class="btn-action btn-view">
                                <i class="fas fa-eye"></i> View Bill
                            </a>
                            <a href="print_receipt.php?payment_id=<?= $payment['id'] ?>" class="btn-action btn-print" target="_blank">
                                <i class="fas fa-print"></i> Receipt
                            </a>
                            <a href="#" class="btn-action btn-pdf" onclick="downloadPDF('<?= $payment['receipt_number'] ?>'); return false;">
...
<script>
    ...
    console.log('%c💰 Braick - Payment History', 'font-size:18px; font-weight:bold; color:#0B5ED7;');
    console.log('%c👤 User: <?= htmlspecialchars($user_full_name) ?>', 'font-size:13px; color:#059669;');
    console.log('%c📊 Total Payments: <?= $total_payments ?> | Total Amount: TSh <?= number_format($total_amount) ?>', 'font-size:13px; color:#0B5ED7;');
</script>
