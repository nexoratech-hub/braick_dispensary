...
// CASHIER - PARTIAL PAYMENTS LIST (FIXED)
// USES bills TABLE (NOT patient_bills)
// WITH DISCOUNT DISPLAY AND PDF EXPORT
// ================================================================
...
// ================================================================
// GET PARTIAL PAYMENT BILLS - USING bills TABLE
// ================================================================
try {
    $sql = "
        SELECT 
            ...
            (SELECT SUM(total_price) FROM bill_items WHERE bill_id = b.id AND status != 'cancelled' AND status != 'paid') as pending_total,
            (SELECT SUM(discount_amount) FROM bill_items WHERE bill_id = b.id AND item_type = 'medication') as med_discount,
            (SELECT SUM(discount_amount) FROM bill_items WHERE bill_id = b.id) as total_discount,
...
<main class="main-content">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5" style="max-width:1200px;margin:0 auto;">
        <div class="stat-card">
            ...
            <p class="stat-number green"><?= $total_partial ?></p>
            <p class="stat-label">Total Partial Bills</p>
        </div>
        <div class="stat-card">
            <div class="stat-icon">📅</div>
        ...
        <div class="stat-card">
            <div class="stat-icon">💰</div>
            <p class="stat-number blue"><?= $currency ?> <?= number_format($total_amount, 0) ?></p>
    ...
    <div class="card" style="max-width:1200px;margin:0 auto;">
        <div class="card-header">
            <h3 class="card-title">
                ...
                <span class="text-sm font-normal text-gray-400">(<?= $total_partial ?> bills)</span>
            </h3>
                ...
                <tbody>
                    <?php if (is_array($partial_bills) && count($partial_bills) > 0): ?>
                        <?php $i = 1; foreach ($partial_bills as $bill): 
                            ...
                            $med_discount = (float)($bill['med_discount'] ?? 0);
                            $bill_discount = $discount > 0 ? $discount : $med_discount;
...
<script>
    ...
    function generatePDF() {
        ...
        ?>
            billsHtml += `
                <tr>
                    ...
                    <td style="padding:3px 8px;border-bottom:1px solid #E2E8F0;text-align:right;font-size:13px;color:<?= $balance > 0 ? '#DC2626' : '#059669' ?>;">${currency} <?= number_format($balance, 0) ?></td>
                    <td style="padding:3px 8px;border-bottom:1px solid #E2E8F0;text-align:right;font-size:13px;color:#D97706;">${ <?= $bill_discount > 0 ? 'currency + " " + ' . number_format($bill_discount, 0) : '"-"' ?> }</td>
        ...
        if (!billsHtml) {
            billsHtml = `<tr><td colspan="8" style="text-align:center;padding:20px;font-size:14px;color:#64748B;">No partial payment bills found</td></tr>`;
        }
        ...
        var html = `
            ...
            <div style="margin-bottom:8px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;margin:4px 0;">
                    <div style="background:#FEF3C7;padding:6px 10px;border-radius:8px;text-align:center;border:1px solid #D97706;">
                        <div style="font-size:18px;font-weight:700;color:#D97706;">${totalPartial}</div>
                        <div style="font-size:10px;color:#64748B;text-transform:uppercase;font-weight:600;">📋 Total Bills</div>
                    </div>
                    <div style="background:#E8F0FE;padding:6px 10px;border-radius:8px;text-align:center;border:1px solid #0B5ED7;">
                    ...
                    <div style="background:#D1FAE5;padding:6px 10px;border-radius:8px;text-align:center;border:1px solid #059669;">
                        <div style="font-size:18px;font-weight:700;color:#059669;">${currency} ${totalAmount.toLocaleString()}</div>
                        <div style="font-size:10px;color:#64748B;text-transform:uppercase;font-weight:600;">💰 Total Amount</div>
                    </div>
                    <div style="background:#D1FAE5;padding:6px 10px;border-radius:8px;text-align:center;border:1px solid #059669;">
                        <div style="font-size:18px;font-weight:700;color:#059669;">${currency} ${totalBalance.toLocaleString()}</div>
                        <div style="font-size:10px;color:#64748B;text-transform:uppercase;font-weight:600;">⚖️ Total Balance</div>
                    </div>
                </div>
                ${totalDiscount > 0 ? `
                <div style="background:#FEF3C7;padding:4px 12px;border-radius:6px;text-align:center;border:1px solid #D97706;margin-top:4px;">
                    <span style="font-weight:600;color:#D97706;">🏷️ Total Discount Applied: ${currency} ${totalDiscount.toLocaleString()}</span>
                </div>
                ` : ''}
            </div>
            ...
            <!-- PARTIAL BILLS TABLE -->
            <div style="margin-bottom:8px;">
                <div class="pdf-section-title"><i class="fas fa-list"></i> Partial Bills (${totalPartial})</div>
                    ...
                    <table class="pdf-table" style="font-size:13px;width:100%;border-collapse:collapse;">
                        <thead>
                            <tr>
                                ...
                                <th style="background:#059669;color:white;padding:4px 8px;text-align:right;font-size:11px;">Balance</th>
                                <th style="background:#059669;color:white;padding:4px 8px;text-align:right;font-size:11px;">Discount</th>
                                <th style="background:#059669;color:white;padding:4px 8px;text-align:left;font-size:11px;">Updated</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${billsHtml}
    ...
    console.log('%c🟡 Braick - Partial Payments (Fixed - bills table)', 'font-size:18px; font-weight:bold; color:#D97706;');
