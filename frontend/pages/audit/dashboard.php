...
// AUDIT ROLE - DASHBOARD V9
// ✅ AUDIT role only (employee wa kawaida)
...
try {
    ...
    // 6. PROCEDURES (for Other Bills calc)
    $sql = "SELECT COALESCE(SUM(bi.total_price - COALESCE(bi.discount_amount, 0)), 0) as total
            FROM bill_items bi
            INNER JOIN bills b ON bi.bill_id = b.id
            WHERE b.status = 'paid'
    ...
    // OTHER BILLS = procedure + registration + equipment (calculated but not displayed as card)
    $stats['other_revenue'] = 
...
console.log('%c🔍 Audit Dashboard V9', 'font-size:18px; font-weight:bold; color:#0B5ED7;');
console.log('%c✅ AUDIT ROLE', 'font-size:13px; color:#34D399; font-weight:bold;');
console.log('%c✅ 8 CARDS (Other Bills removed)', 'font-size:13px; color:#34D399;');
