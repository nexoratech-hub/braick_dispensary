<?php
// ================================================================
// FILE: frontend/pages/audit/view_equipment.php
// AUDIT ROLE - VIEW EQUIPMENT DETAILS (READ ONLY)
// ✅ HAKUNA Edit/Delete buttons
// ✅ VIEW ONLY - maelezo ya equipment
// ✅ Blue theme (#0B5ED7)
// ================================================================
...
if ($_SESSION['role'] !== 'audit') {
    $role = $_SESSION['role'];
    switch ($role) {
        case 'admin': header('Location: /dispensary_system/frontend/pages/admin/audit/view_equipment.php?id=' . ($_GET['id'] ?? 0)); break;
...
// ================================================================
// GET OTHER BATCHES (same equipment_name)
// ================================================================
$other_batches = [];
try {
    $sql = "SELECT 
                e.id, e.batch_number, e.quantity, e.expiry_date, 
                e.status, e.branch_id, e.unit_cost, e.selling_price,
                b.name AS branch_name
            FROM medical_equipment e
            LEFT JOIN branches b ON e.branch_id = b.id
            WHERE e.equipment_name = ? AND e.id != ?
            ORDER BY e.expiry_date ASC";
    $stmt = $db->prepare($sql);
    $stmt->execute([$equipment['equipment_name'], $equipment_id]);
    $other_batches = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {}

// ================================================================
// GET USAGE HISTORY (equipment sales from bill_items)
// ================================================================
$usage_history = [];
try {
    $sql = "SELECT 
            ...
            LIMIT 20";
    $stmt = $db->prepare($sql);
    $stmt->execute([$equipment['equipment_name']]);
...
$reorder_level = (int)($equipment['reorder_level'] ?? 10);
$unit_cost = (float)($equipment['unit_cost'] ?? 0);
$selling_price = (float)($equipment['selling_price'] ?? 0);
...
if (!empty($equipment['expiry_date']) && $equipment['expiry_date'] !== '0000-00-00') {
    $expiry_ts = strtotime($equipment['expiry_date']);
...
// Total usage
$total_used_qty = 0;
$total_used_revenue = 0;
foreach ($usage_history as $u) {
    $total_used_qty += (int)($u['quantity'] ?? 0);
    $total_used_revenue += (float)($u['total_price'] ?? 0);
}
...
<head>
    ...
    <title>View Equipment - <?= htmlspecialchars($equipment['equipment_name'] ?? 'N/A') ?></title>
...
.equipment-avatar i { color: white; }

.equipment-info { flex: 1; min-width: 200px; }
...
.equipment-meta-item i { color: var(--primary); font-size: 0.65rem; }
...
.status-badge.purple { background: var(--purple-bg); color: var(--purple); border: 1px solid var(--purple); }
...
@media (max-width: 768px) {
    ...
    .equipment-avatar { width: 56px; height: 56px; font-size: 1.4rem; }
    .equipment-name { font-size: 1.1rem; }
...
<main class="main-content">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">
                ...
                <strong><?= htmlspecialchars($equipment['branch_name'] ?? 'N/A') ?></strong>
                <span class="branch-tag">
                    <i class="fas fa-hashtag"></i> ID: <?= (int)$equipment['id'] ?>
                </span>
                <?php if (!empty($equipment['batch_number'])): ?>
                    <span class="branch-tag">
                        <i class="fas fa-barcode"></i> Batch: <?= htmlspecialchars($equipment['batch_number']) ?>
                    </span>
                <?php endif; ?>
            </p>
        </div>
    ...
    <div class="info-card">
        <div class="equipment-profile">
            ...
            <div class="equipment-info">
                <h2 class="equipment-name"><?= htmlspecialchars($equipment['equipment_name'] ?? 'N/A') ?></h2>
                <div class="equipment-meta">
                    <span class="equipment-meta-item">
                        <i class="fas fa-tag"></i>
                        <?= htmlspecialchars($equipment['category'] ?? 'N/A') ?>
                    </span>
                    <span class="equipment-meta-item">
                        <i class="fas fa-balance-scale"></i>
                        <?= htmlspecialchars($equipment['unit'] ?? 'units') ?>
                    </span>
                    <?php if (!empty($equipment['batch_number'])): ?>
                        <span class="equipment-meta-item">
                            <i class="fas fa-barcode"></i>
                            <?= htmlspecialchars($equipment['batch_number']) ?>
                        </span>
                    <?php endif; ?>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card green">
            ...
            <div class="stat-sub">
                <i class="fas fa-tag"></i> Per <?= htmlspecialchars($equipment['unit'] ?? 'unit') ?>
            </div>
        </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:18px;">
        ...
        <div class="info-card">
            <div class="card-header">
                <span class="title"><i class="fas fa-info-circle"></i> Equipment Information</span>
            </div>
            <div class="card-body">
                <div class="info-row">
                    <span class="label"><i class="fas fa-hashtag"></i> Equipment ID</span>
                    <span class="value mono">#<?= (int)$equipment['id'] ?></span>
                </div>
                
                <div class="info-row">
                    <span class="label"><i class="fas fa-tools"></i> Name</span>
                    <span class="value"><?= htmlspecialchars($equipment['equipment_name'] ?? 'N/A') ?></span>
                </div>
                
                <div class="info-row">
                    <span class="label"><i class="fas fa-tag"></i> Category</span>
                    <span class="value"><?= htmlspecialchars($equipment['category'] ?? 'N/A') ?></span>
                </div>
                
                <div class="info-row">
                    <span class="label"><i class="fas fa-balance-scale"></i> Unit Type</span>
                    <span class="value"><?= htmlspecialchars($equipment['unit'] ?? 'N/A') ?></span>
                </div>
                
                <?php if (!empty($equipment['batch_number'])): ?>
                <div class="info-row">
                    <span class="label"><i class="fas fa-barcode"></i> Batch #</span>
                    <span class="value mono"><?= htmlspecialchars($equipment['batch_number']) ?></span>
                </div>
                <?php endif; ?>
                ...
                <div class="info-row">
                    ...
                    <span class="value">
                        <span class="status-badge <?= ($equipment['status'] ?? 'active') === 'active' ? 'success' : 'danger' ?>">
                ...
                <div class="info-row">
                    ...
                    <span class="value"><?= htmlspecialchars($equipment['branch_name'] ?? 'N/A') ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="info-card">
            ...
            <div class="card-body">
                <div class="info-row">
                    <span class="label"><i class="fas fa-cubes"></i> Quantity</span>
                ...
                <div class="info-row">
                    ...
                    <span class="value mono">
                        <?php if (!empty($equipment['expiry_date']) && $equipment['expiry_date'] !== '0000-00-00'): ?>
                            <?= date('d M Y', strtotime($equipment['expiry_date'])) ?>
                        <?php else: ?>
        ...
        <div class="info-card">
            ...
            <div class="card-body">
                <div class="info-row">
                    ...
                    <span class="value"><?= htmlspecialchars($equipment['added_by_name'] ?? 'System') ?></span>
                </div>
                
                <?php if (!empty($equipment['added_by_role'])): ?>
                <div class="info-row">
                    <span class="label"><i class="fas fa-user-tag"></i> Role</span>
                    <span class="value">
                        <span class="status-badge primary"><?= strtoupper($equipment['added_by_role']) ?></span>
                    </span>
                </div>
                <?php endif; ?>
                ...
                <div class="info-row">
                    ...
                    <span class="value mono">
                        <?= !empty($equipment['created_at']) ? date('d M Y, H:i', strtotime($equipment['created_at'])) : 'N/A' ?>
                    </span>
                </div>
                
                <?php if (!empty($equipment['updated_at']) && $equipment['updated_at'] !== $equipment['created_at']): ?>
                <div class="info-row">
                    <span class="label"><i class="fas fa-edit"></i> Last Updated</span>
                    <span class="value mono">
                        <?= date('d M Y, H:i', strtotime($equipment['updated_at'])) ?>
                    </span>
                </div>
                <?php endif; ?>
            </div>
    ...
    <div class="info-card">
        <div class="card-header">
            <span class="title">
                ...
                Usage History
                <span style="font-size:0.68rem;font-weight:600;background:rgba(255,255,255,0.2);padding:2px 8px;border-radius:8px;margin-left:6px;">
                    <?= count($usage_history) ?> records
                </span>
            </span>
            <span class="meta">
                <i class="fas fa-cubes"></i> Total Used: <?= number_format($total_used_qty) ?> units
                &nbsp;|&nbsp;
                <i class="fas fa-money-bill-wave"></i> <?= $currency ?> <?= number_format($total_used_revenue, 0) ?>
            </span>
        </div>
        ...
        <div style="overflow-x:auto;">
            <table class="data-table">
                ...
                <tbody>
                    <?php $u_num = 1; foreach ($usage_history as $u): ?>
                        <tr>
                            <td style="text-align:center;font-weight:700;color:var(--text-secondary);"><?= $u_num++ ?></td>
                            <td>
                                <span style="font-family:var(--font-mono);font-size:0.68rem;color:var(--primary);font-weight:800;background:var(--primary-bg);padding:3px 7px;border-radius:5px;">
                                    <?= htmlspecialchars($u['bill_number'] ?? 'N/A') ?>
                                </span>
                            </td>
                            <td style="font-size:0.75rem;font-weight:600;">
                                <?= htmlspecialchars($u['patient_name'] ?? 'Walk-in') ?>
                            </td>
                            <td style="text-align:center;font-weight:800;color:var(--primary);font-family:var(--font-mono);">
                                <?= number_format($u['quantity'] ?? 0) ?>
                            </td>
                            <td class="money-cell">
                                <span class="currency-prefix"><?= $currency ?></span><?= number_format($u['unit_price'] ?? 0, 0) ?>
                            </td>
                            <td class="money-cell" style="color:var(--danger);">
                                <?php if (($u['discount_amount'] ?? 0) > 0): ?>
                                    - <span class="currency-prefix"><?= $currency ?></span><?= number_format($u['discount_amount'], 0) ?>
                                <?php else: ?>
                                    <span style="color:var(--text-secondary);">—</span>
                                <?php endif; ?>
                            </td>
                            <td class="money-cell">
                                <span class="currency-prefix"><?= $currency ?></span><?= number_format($u['total_price'] ?? 0, 0) ?>
                            </td>
                            <td>
                                <div style="font-size:0.68rem;font-weight:600;"><?= date('d M Y', strtotime($u['created_at'])) ?></div>
                                <div style="font-size:0.58rem;color:var(--text-secondary);font-family:var(--font-mono);"><?= date('H:i', strtotime($u['created_at'])) ?></div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <div class="empty-state">
            <i class="fas fa-history"></i>
            <p>No usage recorded for this equipment yet</p>
        </div>
        <?php endif; ?>
    </div>
...
<script>
console.log('%c🔧 AUDIT View Equipment - READ ONLY', 'font-size:18px; font-weight:bold; color:#0B5ED7;');
...
console.log('%c🔧 Equipment: <?= htmlspecialchars($equipment['equipment_name'] ?? 'N/A') ?>', 'font-size:13px; color:#0B5ED7;');
