...
// AUDIT - STOCK MOVEMENT REPORT V8.4 (FULLY FIXED)
// ================================================================
...
// ✅ V8.4: Branch LOCKED - anaona branch yake pekee
...
function getStockBefore($db, $item_type, $item_ids, $date_from_sql, $branch_id) {
    ...
    try {
        ...
        $sql = "SELECT sm.previous_stock
                ...
                LIMIT 1";
        $params = array_merge($item_ids, [$date_from_sql, $date_to_temp, $branch_id]);
        $stmt = $db->prepare($sql);
        $stmt->execute($params);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        // 3. Fallback: chukua quantity ya sasa ya item
...
if ($active_tab === 'medicine' && $selected_item_id > 0) {
    ...
    if ($med_info) {
        ...
        // Additions (IN movements) - bila returns
        $additions = [];
        foreach ($movements as $m) {
...
if (isset($_GET['export']) && $_GET['export'] === 'csv' && $selected_item_id > 0) {
    ...
    if ($details) {
        ...
        fputcsv($output, ['STOCK MOVEMENT REPORT V8.4 - AUDIT']);
...
<title>Stock Movement Report • Braick Audit V8.4</title>
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title">
                ...
                <span class="branch-tag"><i class="fas fa-shield-alt"></i> AUDIT V8.4</span>
                ...
                <div class="sf-flow">
                    ...
                    <div class="sf-block remaining">
                        ...
                        <div class="sf-block-meta">
                            <span><i class="fas fa-calendar"></i> As of <?= date('d/m/Y', strtotime($filter_date_to)) ?> 23:59</span>
