...
// AUDIT - STOCK MOVEMENT REPORT V8.6 (FULLY CORRECTED)
// ================================================================
...
// ✅ V8.6 FIX #6: Branch LOCKED kwa audit user
// ================================================================
...
$branch_params_sm = [$selected_branch_id];
 ...
 */
function getStockBefore($db, $item_type, $item_ids, $date_from_sql, $branch_id) {
    ...
    try {
        ...
        $sql_flow = "SELECT 
                    ...
                    WHERE sm.$id_field IN ($placeholders)
                      AND sm.created_at >= ?
                      AND sm.branch_id = ?";
        $params = array_merge($item_ids, [$date_from_sql, $branch_id]);
...
try {
    ...
    foreach ($raw as $r) {
        $stmt2 = $db->prepare("SELECT medication_name FROM medications_inventory WHERE id = ? AND branch_id = ? LIMIT 1");
        $stmt2->execute([$r['item_id'], $selected_branch_id]);
...
if ($active_tab === 'medicine' && $selected_item_id > 0) {
    ...
    if ($med_info) {
        ...
        $stock_before = getStockBefore($db, 'medicine', $inventory_ids, $date_from_sql, $selected_branch_id);
        
        // ✅ V8.6: stock_after = current stock
...
if ($active_tab === 'equipment' && $selected_item_id > 0) {
    ...
    if ($eq_info) {
        ...
        $stock_before = getStockBefore($db, 'equipment', $equipment_ids, $date_from_sql, $selected_branch_id);
        $stock_after = $total_current_stock;
...
if (isset($_GET['export']) && $_GET['export'] === 'csv' && $selected_item_id > 0) {
    ...
    if ($details) {
        $filename = 'stock_movement_audit_' . preg_replace('/[^a-zA-Z0-9]/', '_', $details['name']) . '_' . $filter_date_from . '_to_' . $filter_date_to . '.csv';
        ...
        fputcsv($output, ['STOCK MOVEMENT REPORT V8.6 - AUDIT']);
...
<title>Stock Movement Report • Braick Audit V8.6</title>
...
.top5-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px; animation: shimmer 3s infinite linear; background-size: 200% 100%; }
.top5-card.medicine::before { background: linear-gradient(90deg, #D97706, #F59E0B, #FCD34D, #F59E0B, #D97706); }
.top5-card.equipment::before { background: linear-gradient(90deg, #0B5ED7, #3B82F6, #93C5FD, #3B82F6, #0B5ED7); }
...
.data-warning-banner .dw-text code { background: rgba(120, 53, 15, 0.15); padding: 2px 6px; border-radius: 4px; font-family: var(--font-mono); font-size: 0.72rem; font-weight: 700; }
...
<main class="main-content">
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title">
                ...
                <span class="branch-tag"><i class="fas fa-shield-alt"></i> AUDIT V8.6</span>
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;position:relative;z-index:1;">
            ...
            <a href="?tab=<?= $active_tab ?>&quick=<?= $quick_filter ?>&item_id=<?= $selected_item_id ?>&date_from=<?= $date_from ?>&date_to=<?= $date_to ?>&export=csv" class="btn-header export">
                <i class="fas fa-file-csv"></i> Export CSV
            </a>
            <?php endif; ?>
            <button onclick="window.print()" class="btn-header"><i class="fas fa-print"></i> Print</button>
            <a href="dashboard.php" class="btn-header"><i class="fas fa-arrow-left"></i> Dashboard</a>
        </div>
    </div>

    <div class="tabs-container">
        <a href="?tab=medicine&quick=<?= $quick_filter ?>&date_from=<?= $date_from ?>&date_to=<?= $date_to ?>" class="tab-btn <?= $active_tab === 'medicine' ? 'active' : '' ?>">
            <i class="fas fa-pills"></i> MEDICINE TRACKING
        </a>
        <a href="?tab=equipment&quick=<?= $quick_filter ?>&date_from=<?= $date_from ?>&date_to=<?= $date_to ?>" class="tab-btn <?= $active_tab === 'equipment' ? 'active' : '' ?>">
    ...
    <div class="filter-card">
        ...
        <div class="quick-filters">
            ...
            <a href="?tab=<?= $active_tab ?>&quick=yesterday<?= $selected_item_id ? '&item_id=' . $selected_item_id : '' ?>" class="quick-btn <?= $quick_filter === 'yesterday' ? 'active' : '' ?>"><i class="fas fa-calendar-minus"></i> Yesterday</a>
    ...
    <div class="top5-card medicine">
        ...
        <div class="top5-grid">
            <?php foreach ($top5_medicines as $idx => $tm): ?>
                <a href="?tab=medicine&quick=<?= $quick_filter ?>&item_id=<?= (int)$tm['item_id'] ?>" class="top5-item medicine">
        ...
        <?php else: ?>
        <div class="top5-empty"><i class="fas fa-inbox"></i><p>No medicine movements in this period</p><p style="font-size:0.68rem;margin-top:4px;opacity:0.7;">Try changing date filter</p></div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="top5-card equipment">
        ...
        <div class="top5-grid">
            <?php foreach ($top5_equipment as $idx => $te): ?>
                <a href="?tab=equipment&quick=<?= $quick_filter ?>&item_id=<?= (int)$te['item_id'] ?>" class="top5-item equipment">
                    <div class="top5-rank">#<?= $idx + 1 ?></div>
        ...
        <?php else: ?>
        <div class="top5-empty"><i class="fas fa-inbox"></i><p>No equipment movements in this period</p><p style="font-size:0.68rem;margin-top:4px;opacity:0.7;">Try changing date filter</p></div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <?php if ($active_tab === 'medicine'): ?>
        <?php if ($medicine_details): ?>
            ...
            <div class="stock-flow-card">
                ...
                <div style="text-align: center; padding: 14px; background: var(--bg-body); border-radius: var(--radius-md); font-family: var(--font-mono); font-weight: 800; border: 2px dashed var(--border-color); font-size: 0.95rem;">
                    ...
                    <span style="color:#059669;font-size:1.15rem;" title="Total IN"><?= number_format($total_in_display) ?></span>
                    <span style="color:var(--text-secondary);margin:0 8px;">−</span>
                    <span style="color:#DC2626;font-size:1.15rem;" title="Total OUT"><?= number_format($total_out_display) ?></span>
                    <span style="color:var(--text-secondary);margin:0 8px;">=</span>
        ...
        <?php else: ?>
            <div class="table-card">
                ...
                <div class="empty-state"><i class="fas fa-search"></i><p>Search medicine above or click on Top 5 cards</p><p style="font-size:0.78rem;margin-top:8px;color:var(--text-muted);">Stock Flow • Additions • Movements • Performed By</p></div>
            </div>
        <?php endif; ?>
    ...
    <?php else: ?>
            ...
            <div class="stock-flow-card">
                ...
                <div class="sf-flow">
                    ...
                    <div class="sf-block remaining">
                        ...
                        <div class="sf-block-value"><?= number_format($pi['stock_after']) ?> <span class="unit">units</span></div>
                        <div class="sf-block-meta">
                            <span><i class="fas fa-database"></i> Current live: <?= number_format($equipment_details['total_current_stock']) ?></span>
                        </div>
                    </div>
                </div>
        ...
        <?php else: ?>
            <div class="table-card">
                ...
                <div class="empty-state"><i class="fas fa-search"></i><p>Search equipment above or click on Top 5 cards</p><p style="font-size:0.78rem;margin-top:8px;color:var(--text-muted);">Stock Flow • Additions • Movements • Performed By</p></div>
            </div>
        <?php endif; ?>
