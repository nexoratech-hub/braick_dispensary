<?php
// ================================================================
// FILE: frontend/pages/audit/view_payment.php
// AUDIT - VIEW PAYMENT DETAILS (V3 - BRANCH LOCKED, NO EDIT)
// ✅ Sawa na admin/audit/view_payment.php design
...
// ================================================================
// GET PAYMENT DETAILS - ✅ BRANCH YA ALIYE LOGIN TU
// ================================================================
$payment = null;
try {
    $sql = "SELECT 
...
if (!$payment) {
    $_SESSION['error_message'] = "Payment not found or you don't have permission to view this payment (different branch).";
    header('Location: other_services.php?tab=otc_bills');
    exit;
}
...
<head>
    ...
    <title>View Payment - <?= htmlspecialchars($payment['receipt_number'] ?? 'N/A') ?> - AUDIT</title>
...
.audit-badge { background: linear-gradient(135deg, #3B82F6, #2563EB); color: white; padding: 4px 14px; border-radius: 20px; font-size: 0.65rem; font-weight: 800; letter-spacing: 0.05em; border: 1px solid rgba(255,255,255,0.3); display: inline-flex; align-items: center; gap: 5px; }
...
/* BILL ITEMS - GREEN THEME */
.items-table-card { background: var(--bg-card); border-radius: 14px; border: 2px solid #059669; overflow: hidden; box-shadow: 0 4px 12px rgba(5, 150, 105, 0.12); margin-bottom: 18px; }
.items-table-card:hover { box-shadow: 0 8px 24px rgba(5, 150, 105, 0.2); border-color: #047857; }
.items-table-card .card-header { padding: 14px 20px; background: linear-gradient(135deg, #059669, #047857); display: flex; justify-content: space-between; align-items: center; gap: 8px; flex-wrap: wrap; }
.items-table-card .card-header .title { color: white; font-size: 0.9rem; font-weight: 800; display: flex; align-items: center; gap: 8px; }
.items-table-card .card-header .title i { color: #A7F3D0; font-size: 1rem; }
...
.items-table-card .card-header .count { color: rgba(255,255,255,0.95); font-size: 0.7rem; font-weight: 700; background: rgba(255,255,255,0.2); padding: 4px 12px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.2); }
...
.items-scroll-wrapper::-webkit-scrollbar-track { background: rgba(5, 150, 105, 0.1); border-radius: 10px; margin: 0 8px; }
.items-scroll-wrapper::-webkit-scrollbar-thumb { background: linear-gradient(90deg, #059669, #10B981); border-radius: 10px; border: 2px solid var(--bg-card); }
.items-scroll-wrapper::-webkit-scrollbar-thumb:hover { background: linear-gradient(90deg, #047857, #059669); }

/* GREEN DATA TABLE */
.data-table-green { width: 100%; border-collapse: collapse; font-size: 0.78rem; min-width: 1000px; }
.data-table-green thead th { text-align: left; padding: 10px 12px; font-weight: 800; font-size: 0.6rem; text-transform: uppercase; letter-spacing: 0.06em; color: white; background: linear-gradient(135deg, #059669, #047857); white-space: nowrap; }
.data-table-green tbody td { padding: 10px 12px; border-bottom: 1px solid var(--border-color); color: var(--text-primary); vertical-align: middle; font-weight: 500; }
.data-table-green tbody tr:hover td { background: rgba(5, 150, 105, 0.08); }
[data-theme="dark"] .data-table-green tbody tr:hover td { background: rgba(5, 150, 105, 0.15); }
.data-table-green tbody tr:last-child td { border-bottom: none; }

.money-cell-green { font-family: var(--font-mono); font-weight: 800; font-size: 0.8rem; color: var(--success); text-align: right; letter-spacing: -0.02em; }
.money-cell-green .currency-prefix { font-size: 0.65rem; color: var(--text-secondary); margin-right: 2px; font-family: var(--font-primary); font-weight: 600; }

.item-type-badge-green { display: inline-block; padding: 2px 7px; border-radius: 5px; font-size: 0.55rem; font-weight: 800; text-transform: uppercase; background: rgba(5, 150, 105, 0.12); color: #059669; border: 1px solid rgba(5, 150, 105, 0.3); }
...
.data-table-green tbody tr.total-row-green td { border-top: 3px solid #059669; font-weight: 800; background: rgba(5, 150, 105, 0.08) !important; color: #059669; }
...
<main class="main-content">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">
                ...
                <span class="branch-tag">
                    <i class="fas fa-store-alt"></i> <?= htmlspecialchars($payment['branch_name'] ?? $user_branch_name) ?>
                </span>
                <span class="branch-tag">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;position:relative;z-index:1;">
            ...
            <a href="view_bill.php?id=<?= $payment['bill_id'] ?>" class="btn-header">
    ...
    <div class="action-bar">
        ...
        <div class="action-buttons">
            ...
            <?php if (!empty($payment['bill_id'])): ?>
            <a href="view_bill.php?id=<?= $payment['bill_id'] ?>" class="btn btn-secondary">
...
console.log('%c🧾 View Payment - AUDIT (BRANCH LOCKED)', 'font-size:18px; font-weight:bold; color:#0B5ED7;');
console.log('%c✅ AUDIT ANAONA BRANCH YAKE TU', 'font-size:12px; color:#34D399; font-weight:bold;');
console.log('%c✅ HAKUNA Edit button (VIEW ONLY)', 'font-size:12px; color:#34D399; font-weight:bold;');
console.log('%c✅ GREEN Theme kwa Items table', 'font-size:12px; color:#059669; font-weight:bold;');
console.log('%c👥 Branch: <?= htmlspecialchars($payment['branch_name'] ?? $user_branch_name) ?>', 'font-size:12px; color:#0B5ED7; font-weight:bold;');
