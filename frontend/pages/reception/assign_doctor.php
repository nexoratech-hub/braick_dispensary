...
try {
    ...
    // Get patients in this branch without assigned doctor
    $stmt = $db->prepare("
        SELECT p.* 
        FROM patients p
        LEFT JOIN visits v ON p.id = v.patient_id AND v.status = 'pending'
        WHERE p.branch_id = ? 
        AND p.id NOT IN (
            SELECT patient_id FROM visits WHERE status IN ('pending', 'assigned', 'with_doctor') AND branch_id = ?
        )
...
<style>
    ...
    .patient-list-card .name {
        font-weight: 500;
    }
    .patient-list-card .id {
        font-size: 0.7rem;
        color: var(--text-secondary);
    }
</style>
...
<main class="main-content">
        ...
        <div>
            ...
            <p class="page-subtitle">
                ...
                <span class="ml-2 inline-flex bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs border border-blue-200">
                    <i class="fas fa-user mr-1"></i> <?= count($patients) ?> patients waiting
                </span>
            </p>
        </div>
        <div>
    ...
    <div class="card mb-5">
        <div class="card-header">
            <h3 class="card-title">
                <i class="fas fa-user-injured title-blue mr-2"></i> Patients Waiting for Doctor
                <span class="text-sm font-normal text-gray-400">(<?= count($patients) ?> patients)</span>
            </h3>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            <?php foreach ($patients as $patient): ?>
                <div class="patient-list-card">
                    ...
                    <a href="assign_doctor.php?patient_id=<?= $patient['id'] ?>" class="btn btn-blue btn-sm">
    ...
    <div class="form-card">
        ...
        <form method="POST" action="">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="form-row">
                    ...
                    <?php if (empty($patients)): ?>
                        <p class="text-xs text-gray-400 mt-1">All patients in <?= htmlspecialchars($branch_name) ?> have been assigned a doctor.</p>
                    <?php endif; ?>
                </div>
                ...
                <div class="form-row">
                    ...
                    <?php if (empty($doctors)): ?>
                        <p class="text-xs text-gray-400 mt-1">No doctors available in <?= htmlspecialchars($branch_name) ?>.</p>
                    <?php endif; ?>
                </div>
                ...
                <div class="form-row md:col-span-2">
                    ...
                    <textarea name="symptoms" class="form-control" placeholder="Describe patient symptoms..."></textarea>
                </div>
            ...
            <div class="form-actions">
                <button type="submit" class="btn btn-blue" <?= (empty($patients) || empty($doctors)) ? 'disabled' : '' ?>>
...
<script>
    ...
    console.log('%c👨‍⚕️ Braick - Assign Doctor (Branch Filtered)', 'font-size:18px; font-weight:bold; color:#0B5ED7;');
    console.log('%c🏢 Branch: <?= htmlspecialchars($branch_name) ?>', 'font-size:13px; color:#059669;');
    console.log('%c👥 Patients Waiting: <?= count($patients) ?>', 'font-size:13px; color:#64748B;');
