...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* ================================================================ */
        /* BILL TABLE - FIXED: Bill Number wraps to 2 rows */
        /* ================================================================ */
        .bill-number-cell {
            max-width: 80px;
        ...
        .bill-items-cell .item-row:last-child {
            border-bottom: none;
        }
        
        .bill-items-cell .item-name {
            display: inline-block;
            max-width: 80px;
            white-space: nowrap;
            overflow: hidden;
        ...
        .bill-items-cell .item-qty {
            color: #94A3B8;
            margin: 0 2px;
        }
        
        .bill-items-cell .item-price {
            float: right;
            font-weight: 500;
        }
...
<body>
    ...
    <div class="pdf-container" id="pdfContent">
        ...
        <!-- ================================================================ -->
        <!-- 9. BILLS WITH SUMMARY CARDS - FIXED: Bill Number wraps to 2 rows -->
        <!-- ================================================================ -->
        <div class="section-title">
            💰 Bills & Payments
        ...
        <!-- Bill Items Table - FIXED: Bill Number wraps to 2 rows -->
        <?php if (count($bills) > 0): ?>
        <table class="data-table">
            <thead>
                <tr>
                    <th style="width:10%;">#</th>
                    <th style="width:18%;">Bill Number</th>
            ...
            <tbody>
                ...
                <tr>
                        ...
                        <?php 
                            ...
                            if (strlen($bill_num) > 20) {
                                ...
                                if (count($parts) >= 3) {
                                    ...
                                    $part2 = implode('-', array_slice($parts, 2));
                                    echo '<span style="font-family:monospace;font-size:6.5pt;font-weight:600;color:#0B5ED7;display:block;line-height:1.3;">' . $part1 . '<br>' . $part2 . '</span>';
                                } else {
                                    echo '<span style="font-family:monospace;font-size:6.5pt;font-weight:600;color:#0B5ED7;">' . $bill_num . '</span>';
                                }
                            } else {
                                echo '<span style="font-family:monospace;font-size:6.5pt;font-weight:600;color:#0B5ED7;">' . $bill_num . '</span>';
                            }
                    ...
                    <td>
                        <?php if (count($items) > 0): ?>
                            <?php foreach ($items as $item): ?>
                                <div style="font-size:6.5pt;padding:1px 0;border-bottom:1px solid #F1F5F9;line-height:1.3;">
                                    <span style="display:inline-block;max-width:80px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= htmlspecialchars($item['item_name'] ?? '') ?></span>
                    ...
                    <td class="font-semibold" style="font-size:8pt;">TSh <?= number_format($bill['total_amount'] ?? 0, 0) ?></td>
                    <td style="font-size:8pt;">TSh <?= number_format($bill['paid_amount'] ?? 0, 0) ?></td>
                    <td style="font-weight:600;font-size:8pt;color:<?= ($bill['balance'] ?? 0) > 0 ? '#DC2626' : '#059669' ?>;">
