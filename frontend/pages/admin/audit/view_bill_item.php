<?php
// ================================================================
// FILE: frontend/pages/admin/audit/view_bill_item.php
// ADMIN AUDIT - VIEW BILL ITEM DETAILS
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'delete_item') {
        try {
            $stmt = $db->prepare("SELECT item_name, bill_id, visit_id FROM bill_items bi LEFT JOIN bills b ON bi.bill_id = b.id WHERE bi.id = ?");
            $stmt->execute([$item_id]);
            $item_del = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($item_del) {
                $bill_id = (int)$item_del['bill_id'];
                $visit_id_del = (int)$item_del['visit_id'];
                ...
                if ($visit_id_del > 0) {
                    header('Location: view_visit.php?id=' . $visit_id_del . '&branch=' . $selected_branch_id . '&deleted=1');
                } else {
...
.header-badge.pink { background: linear-gradient(135deg, #DB2777, #F472B6); border-color: rgba(255,255,255,0.25); font-weight: 800; }
...
.related-card-header {
    padding: 12px 18px;
    background: linear-gradient(135deg, var(--slate-bg), transparent);
    border-bottom: 1px solid var(--border-color);
    display: flex;
    justify-content: space-between;
    align-items: center;
...
.related-card-title i { color: var(--primary); }
...
<main class="main-content">
    ...
    <div class="page-header">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;position:relative;z-index:1;">
            <a href="edit_bill_item.php?id=<?= $item_id ?>&branch=<?= $selected_branch_id ?>" class="btn-header warning">
    ...
    <div class="card">
        <div class="card-header indigo">
            ...
            <a href="view_visit.php?id=<?= $item['visit_id'] ?>&branch=<?= $selected_branch_id ?>" style="color:white;font-size:0.7rem;font-weight:700;text-decoration:none;background:rgba(255,255,255,0.2);padding:5px 12px;border-radius:var(--radius-full);backdrop-filter:blur(10px);display:inline-flex;align-items:center;gap:5px;">
