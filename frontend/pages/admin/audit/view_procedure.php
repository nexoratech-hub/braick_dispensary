<?php
// ================================================================
// FILE: frontend/pages/admin/audit/view_procedure.php
// ADMIN - VIEW PROCEDURE / EQUIPMENT DETAILS
// ✅ Inatumia reference_id + bill_item_id
// ✅ Inaonyesha bill info sahihi
// ✅ Inaonyesha payments
// ✅ BLUE THEME
// ================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_procedure') {
    if (!$is_admin) {
        ...
        try {
            ...
            // Fetch bill_item
            $stmt = $db->prepare("
                SELECT bi.*, b.bill_number, b.id as bill_id, b.visit_id
                FROM bill_items bi
            ...
            $db->prepare("DELETE FROM bill_items WHERE id = ?")->execute([$del_bill_item_id]);
            ...
            // Delete from procedures table kama ni procedure
            if ($del_item_type === 'procedure' && $del_reference_id > 0) {
                $db->prepare("DELETE FROM procedures WHERE id = ?")->execute([$del_reference_id]);
            }
            
            // Log
            logActivity(
                $db, $user_id, $bill_item['branch_id'] ?? null, $bill_item['patient_id'] ?? null,
                'bill_item_deleted',
                "Deleted {$del_item_type}: '{$proc_name}' from Bill #{$bill_number}. Bill recalculated."
            );
            
            $db->commit();
            $_SESSION['success_message'] = "'{$proc_name}' deleted successfully. Bill #{$bill_number} updated.";
            ...
            $alert_message = "Delete failed: " . $e->getMessage();
            $alert_type = 'error';
        }
    }
}
...
.btn-edit { background: linear-gradient(135deg, #D97706, #B45309); color: white; box-shadow: 0 4px 12px rgba(217, 119, 6, 0.3); }
.btn-edit:hover { box-shadow: 0 6px 20px rgba(217, 119, 6, 0.5); color: white; }
.btn-delete { background: linear-gradient(135deg, #DC2626, #B91C1C); color: white; box-shadow: 0 4px 12px rgba(220, 38, 38, 0.3); }
.btn-delete:hover { box-shadow: 0 6px 20px rgba(220, 38, 38, 0.5); color: white; }
...
.modal-icon { width: 68px; height: 68px; border-radius: 50%; background: var(--danger-bg); color: var(--danger); display: flex; align-items: center; justify-content: center; font-size: 1.8rem; margin: 0 auto 16px; }
...
.modal-info { background: var(--warning-bg); padding: 12px 16px; border-radius: 10px; margin-bottom: 18px; font-size: 0.75rem; color: var(--warning); font-weight: 600; display: flex; align-items: flex-start; gap: 8px; text-align: left; border-left: 3px solid var(--warning); }
...
@media print {
    .action-bar, .btn-header, .btn-edit, .btn-delete { display: none !important; }
...
<main class="main-content">
    ...
    <div class="action-bar">
        <div class="action-info">
            ...
            <div class="info-text">
                <div class="info-title"><?= $item_label ?> Actions</div>
                <div class="info-sub">
                    <?php if ($is_admin): ?>
                        View, edit, or delete this <?= strtolower($item_label) ?>
                    <?php else: ?>
        ...
        <div class="action-buttons">
            ...
            <?php if ($is_admin): ?>
                <a href="edit_procedure.php?id=<?= $item_id ?>&bill_item_id=<?= $bill_item_row_id ?>&type=<?= $item['item_type'] ?>&branch=<?= $selected_branch_id ?>" 
                   class="btn btn-edit">
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal-box">
        ...
        <h3 class="modal-title">Delete <?= $item_label ?>?</h3>
        <p class="modal-text">
            Are you sure you want to delete this <?= strtolower($item_label) ?>?<br>
            <strong><?= htmlspecialchars($item['procedure_name']) ?></strong><br>
            <span style="font-size:0.75rem;color:var(--text-secondary);">
                Patient: <strong><?= htmlspecialchars($item['patient_name'] ?? 'N/A') ?></strong>
            </span>
        </p>
        ...
        <div class="modal-info">
            ...
            <span>
                ...
                • Bill item will be removed from Bill <strong><?= htmlspecialchars($item['bill_number'] ?? 'N/A') ?></strong><br>
                • Bill total will decrease by <strong><?= $currency ?> <?= number_format($item['procedure_total'] ?? 0, 0) ?></strong><br>
                • Bill status will be updated automatically<br>
        ...
        <form method="POST" id="deleteForm">
            ...
            <input type="hidden" name="bill_item_id" value="<?= $bill_item_row_id ?>">
            <input type="hidden" name="reference_id" value="<?= $item_id ?>">
...
console.log('%c💉 View <?= $item_label ?> - Admin', 'font-size:18px;font-weight:bold;color:#0B5ED7;');
