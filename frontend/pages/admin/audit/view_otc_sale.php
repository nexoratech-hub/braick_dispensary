...
// ADMIN - VIEW OTC SALE DETAILS (BLUE THEME) - V8
...
// ✅ Delete Whole Sale
// ✅ Load ONLY items with quantity > 0 (skip zero-quantity items)
// ✅ Scroll buttons <>
// ✅ BLUE THEME (#0B5ED7)
// ================================================================
    ...
    if ($user_role === 'audit') {
        ...
        if ($_POST['action'] === 'delete_otc_sale') {
            ...
            try {
                ...
                // ✅ STOCK LOGIC (only items with qty > 0)
                $stock_returned_count = 0;
                $stock_not_returned_count = 0;
                
                if ($payment_status !== 'paid' && $payment_status !== 'dispensed') {
                    $stmt = $db->prepare("SELECT inventory_id, quantity FROM otc_sale_items WHERE sale_id = ? AND quantity > 0");
                    $stmt->execute([$del_sale_id]);
...
console.log('%c🛒 View OTC Sale V8 - SKIP ZERO QTY ITEMS', 'font-size:18px; font-weight:bold; color:#0B5ED7;');
