...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
            ...
            if ($bill) {
                ...
                try {
                    $db->prepare("INSERT INTO activity_logs (user_id, branch_id, action, details, ip_address, created_at) 
                       ...
                       ->execute([$user_id, $selected_branch_id !== 'all' ? (int)$selected_branch_id : $user_branch_id,
                           "Deleted bill: " . ($bill['bill_number'] ?? 'N/A') . " - Amount: " . ($bill['total_amount'] ?? 0),
                           $_SERVER['REMOTE_ADDR'] ?? 'unknown']);
                } catch (Exception $e) {}
                
                $alert_message = "Bill " . ($bill['bill_number'] ?? 'N/A') . " deleted successfully!";
                $alert_type = 'success';
            }
        } catch (Exception $e) {
            $alert_message = "Error deleting bill: " . $e->getMessage();
            $alert_type = 'error';
        }
    }
            ...
            if ($sale) {
                ...
                try {
                    $db->prepare("INSERT INTO activity_logs (user_id, branch_id, action, details, ip_address, created_at) 
                       ...
                       ->execute([$user_id, $selected_branch_id !== 'all' ? (int)$selected_branch_id : $user_branch_id,
                           "Deleted OTC sale: " . ($sale['sale_number'] ?? 'N/A') . " - Amount: " . ($sale['total_amount'] ?? 0),
                           $_SERVER['REMOTE_ADDR'] ?? 'unknown']);
                } catch (Exception $e) {}
                
                $alert_message = "OTC Sale " . ($sale['sale_number'] ?? 'N/A') . " deleted successfully!";
                $alert_type = 'success';
            }
        } catch (Exception $e) {
    ...
    if ($_POST['action'] === 'delete_expense' && !empty($_POST['expense_id'])) {
        try {
            ...
            $stmt = $db->prepare("SELECT expense_number, amount, category FROM expenses WHERE id = ?");
            ...
            if ($expense) {
                ...
                try {
                    $db->prepare("INSERT INTO activity_logs (user_id, branch_id, action, details, ip_address, created_at) 
                                  VALUES (?, ?, 'delete_expense', ?, ?, NOW())")
                       ->execute([$user_id, $selected_branch_id !== 'all' ? (int)$selected_branch_id : $user_branch_id,
                           "Deleted expense: " . ($expense['expense_number'] ?? 'N/A') . " - " . ($expense['category'] ?? '') . " - Amount: " . ($expense['amount'] ?? 0),
                           $_SERVER['REMOTE_ADDR'] ?? 'unknown']);
                } catch (Exception $e) {}
                
                $alert_message = "Expense " . ($expense['expense_number'] ?? 'N/A') . " deleted successfully!";
                $alert_type = 'success';
            }
        } catch (Exception $e) {
            $alert_message = "Error deleting expense: " . $e->getMessage();
            $alert_type = 'error';
        }
    }
}
...
.money-number, .stat-value, .money-cell, .card-value,
.rank-badge, .bill-number, .font-mono {
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <?php if ($is_otc): ?>
                                            ...
                                            <a href="edit_otc.php?id=<?= $trans['id'] ?>&branch=<?= $selected_branch_id ?>" 
                                               class="btn-action edit" title="Edit OTC Sale">
                                        ...
                                        <?php else: ?>
                                            ...
                                            <a href="edit_bill.php?id=<?= $trans['id'] ?>&branch=<?= $selected_branch_id ?>" 
                                               class="btn-action edit" title="Edit Bill">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <button type="button" class="btn-action delete" title="Delete Bill"
                                                    onclick="confirmDelete('bill', <?= $trans['id'] ?>, '<?= htmlspecialchars(addslashes($trans['reference_number'] ?? 'N/A')) ?>')">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                        <?php endif; ?>
                                    </div>
            ...
            <table class="data-table" id="expTable" style="min-width:1300px;">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td class="searchable-cell">
                                <div class="received-by">
                                    ...
                                    <div class="received-by-info">
                                        ...
                                        <span class="received-by-role">
                                            <span class="role-tag <?= htmlspecialchars($role) ?>"><?= htmlspecialchars(strtoupper($role)) ?></span>
                                            <?php if (!empty($exp['recorded_by_username'])): ?>
                                                <span style="font-size:0.55rem;color:var(--text-secondary);font-weight:600;">@<?= htmlspecialchars($exp['recorded_by_username']) ?></span>
                                            <?php endif; ?>
                                        </span>
                                    </div>
                                </div>
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <a href="edit_expense.php?id=<?= $exp['id'] ?>&branch=<?= $selected_branch_id ?>" 
                                       class="btn-action edit" title="Edit Expense">
                                        <i class="fas fa-edit"></i>
                                    </a>
                    ...
                    <tr class="total-row">
                        ...
                        <td colspan="2" style="text-align:center;font-weight:800;color:#DC2626;font-size:0.75rem;">
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: { 
                        ...
                        titleFont: { family: 'Inter', size: 12, weight: 'bold' }, bodyFont: { family: 'JetBrains Mono', size: 11, weight: '600' },
