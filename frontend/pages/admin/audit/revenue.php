...
// ADMIN AUDIT - REVENUE REPORT (V52 - FINAL)
// ✅ V52: Prescription = GROSS pekee (bila discount, bila premium)
// ✅ V51: Custom Date Range Filter (date_from + date_to + presets)
// ✅ V50: Cards 8 TU + Wide DP Card
// ✅ V50: Design inafanana na Audit Dashboard
// ✅ OTC Sales: Kila item kwenye row yake + scroll arrows
...
// ================================================================
// ✅ V51: FILTERS — CUSTOM DATE RANGE ADDED
// ================================================================
...
foreach ($breakdown_types as $type) {
    ...
    try {
        ...
        $stmt->execute(array_merge(
            [$type], 
            $branch_params_p, $date_params, $pay_params,
            $branch_params_bi
        ));
        $data = $stmt->fetch(PDO::FETCH_ASSOC);
        $breakdown_data[$type] = [
...
// ================================================================
// ✅ V52 FIX: PRESCRIPTION = GROSS PEKEE (bila discount, bila premium)
// ================================================================
$prescription_gross = $medication_revenue;   // GROSS (total_price bila discount)
$prescription_revenue = $prescription_gross; // GROSS pekee
...
// Breakdown Total = GROSS (bila discount, bila premium)
...
$total_expenses = roundTo50($total_expenses ?? 0);
$net_profit = roundTo50(($total_revenue ?? 0) - ($total_expenses ?? 0));
...
/* ================================================================
   ✅ V51: CUSTOM DATE RANGE FILTER STYLES
   ================================================================ */
#customFilterSection {
    animation: slideDownFilter 0.35s cubic-bezier(0.4, 0, 0.2, 1);
}
...
#customFilterSection input[type="date"]::-webkit-calendar-picker-indicator {
    cursor: pointer;
    filter: invert(0.5);
    transition: filter 0.2s;
}

#customFilterSection input[type="date"]::-webkit-calendar-picker-indicator:hover {
    filter: invert(0.3);
}

[data-theme="dark"] #customFilterSection input[type="date"]::-webkit-calendar-picker-indicator {
    filter: invert(0.8);
}

[data-theme="dark"] #customFilterSection input[type="date"]::-webkit-calendar-picker-indicator:hover {
    filter: invert(1);
}

/* ================================================================
   ✅ V50: COMPACT STATS GRID (8 CARDS TU)
   ================================================================ */
.stats-grid-8 {
    display: grid;
...
/* ================================================================
   ✅ V50: DISCOUNT & PREMIUM CARD MOJA YENYE WIDTH KUBWA
   ================================================================ */
...
@media (max-width: 768px) {
    ...
    #customFilterSection form {
        flex-direction: column;
        align-items: stretch !important;
    }
    
    #customFilterSection form > div {
        min-width: 100% !important;
    }
    
    #customFilterSection form > button {
        width: 100%;
    }
    
    #customFilterSection form > div:last-child {
...
[data-theme="dark"] .patient-group-card:hover { border-color: #60A5FA; box-shadow: 0 12px 40px rgba(59, 130, 246, 0.35), 0 0 0 1px rgba(59, 130, 246, 0.3); }
...
.otc-sale-row { background: linear-gradient(135deg, rgba(8, 145, 178, 0.05), rgba(8, 145, 178, 0.02)); }
.otc-sale-row td { border-top: 2px solid var(--cyan); font-weight: 700; }
.otc-sale-row:hover td { background: var(--cyan-bg) !important; }

.otc-item-row td { background: var(--bg-card); font-size: 0.72rem; }
.otc-item-row:hover td { background: var(--cyan-bg) !important; }
.otc-item-name { display: flex; align-items: center; gap: 6px; font-weight: 700; color: var(--text-primary); }
.otc-item-name i { color: var(--cyan); font-size: 0.6rem; }

.otc-qty-badge { display: inline-flex; align-items: center; justify-content: center; min-width: 32px; padding: 3px 8px; border-radius: 5px; background: var(--cyan-bg); color: var(--cyan); font-family: var(--font-mono); font-weight: 800; font-size: 0.72rem; border: 1px solid rgba(8, 145, 178, 0.25); }
...
.modal-icon { width: 72px; height: 72px; border-radius: 50%; background: var(--danger-bg); color: var(--danger); display: flex; align-items: center; justify-content: center; font-size: 2rem; margin: 0 auto 18px; animation: iconPulse 1.5s infinite; }
@keyframes iconPulse { 0%, 100% { transform: scale(1); box-shadow: 0 0 0 0 rgba(220, 38, 38, 0.4); } 50% { transform: scale(1.05); box-shadow: 0 0 0 14px rgba(220, 38, 38, 0); } }
...
@media print {
    .filter-card, .btn-header, .visit-action-buttons, .action-buttons, .modal-overlay, .header-scroll-buttons { display: none !important; }
...
<main class="main-content">
    ...
    <!-- ================================================================
         ✅ V51: FILTER CARD - WITH CUSTOM DATE RANGE
         ================================================================ -->
    <div class="filter-card">
        <div class="filter-section">
            <div class="filter-section-title"><i class="fas fa-bolt"></i> Quick Filters</div>
            <div class="quick-filters">
                <a href="?branch=<?= $selected_branch_id ?>&quick=today" class="quick-btn <?= $quick_filter === 'today' ? 'active' : '' ?>"><i class="fas fa-calendar-day"></i> Today</a>
                <a href="?branch=<?= $selected_branch_id ?>&quick=yesterday" class="quick-btn <?= $quick_filter === 'yesterday' ? 'active' : '' ?>"><i class="fas fa-calendar-minus"></i> Yesterday</a>
                <a href="?branch=<?= $selected_branch_id ?>&quick=1d" class="quick-btn <?= $quick_filter === '1d' ? 'active' : '' ?>"><i class="fas fa-clock"></i> 24H</a>
                <a href="?branch=<?= $selected_branch_id ?>&quick=1w" class="quick-btn <?= $quick_filter === '1w' ? 'active' : '' ?>"><i class="fas fa-calendar-week"></i> 1W</a>
                <a href="?branch=<?= $selected_branch_id ?>&quick=1m" class="quick-btn <?= $quick_filter === '1m' ? 'active' : '' ?>"><i class="fas fa-calendar-alt"></i> 1M</a>
                <a href="?branch=<?= $selected_branch_id ?>&quick=3m" class="quick-btn <?= $quick_filter === '3m' ? 'active' : '' ?>"><i class="fas fa-calendar-alt"></i> 3M</a>
                <a href="?branch=<?= $selected_branch_id ?>&quick=6m" class="quick-btn <?= $quick_filter === '6m' ? 'active' : '' ?>"><i class="fas fa-calendar-alt"></i> 6M</a>
                <a href="?branch=<?= $selected_branch_id ?>&quick=1y" class="quick-btn <?= $quick_filter === '1y' ? 'active' : '' ?>"><i class="fas fa-calendar"></i> 1Y</a>
                <a href="?branch=<?= $selected_branch_id ?>&quick=all" class="quick-btn <?= $quick_filter === 'all' ? 'active' : '' ?>"><i class="fas fa-infinity"></i> All</a>
        ...
        <!-- ✅ V51: CUSTOM DATE RANGE FILTER -->
        <div class="filter-section" id="customFilterSection" style="display: <?= $quick_filter === 'custom' ? 'block' : 'none' ?>; margin-top: 14px; padding-top: 14px; border-top: 2px dashed var(--border-color);">
            <div class="filter-section-title"><i class="fas fa-calendar-alt"></i> Custom Date Range</div>
            <form method="GET" action="" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
                ...
                <div style="display: flex; flex-direction: column; gap: 6px; min-width: 180px;">
                    <label style="font-size: 0.62rem; font-weight: 800; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.05em;">
                        <i class="fas fa-calendar-day"></i> From Date
                    </label>
                    <input type="date" name="date_from" id="customDateFrom" 
                           value="<?= htmlspecialchars($date_from) ?>" 
                           max="<?= date('Y-m-d') ?>"
                           style="padding: 9px 12px; border: 2px solid var(--border-color); border-radius: 8px; font-size: 0.82rem; font-family: var(--font-mono); font-weight: 700; background: var(--bg-card); color: var(--text-primary); outline: none; transition: all 0.25s;"
                           onfocus="this.style.borderColor='var(--primary)'; this.style.boxShadow='0 0 0 3px rgba(11, 94, 215, 0.15)';"
                ...
                <div style="display: flex; flex-direction: column; gap: 6px; min-width: 180px;">
                    ...
                    <input type="date" name="date_to" id="customDateTo" 
                ...
                <button type="submit" class="quick-btn active" style="height: 40px; padding: 0 20px;">
                    <i class="fas fa-search"></i> Apply Filter
                </button>
                
                <button type="button" class="quick-btn" onclick="resetCustomFilter();" style="height: 40px; padding: 0 20px;">
                    <i class="fas fa-redo"></i> Reset
                </button>
                
                <div style="display: flex; gap: 6px; flex-wrap: wrap; margin-left: auto;">
                    <button type="button" class="quick-btn" onclick="setDateRange(7);" style="font-size: 0.65rem; padding: 6px 10px;">
                        <i class="fas fa-calendar-week"></i> 7 Days
                    </button>
                    <button type="button" class="quick-btn" onclick="setDateRange(14);" style="font-size: 0.65rem; padding: 6px 10px;">
                        <i class="fas fa-calendar-alt"></i> 14 Days
                    </button>
                    <button type="button" class="quick-btn" onclick="setDateRange(30);" style="font-size: 0.65rem; padding: 6px 10px;">
                        <i class="fas fa-calendar"></i> 30 Days
                    </button>
                    <button type="button" class="quick-btn" onclick="setMonthRange();" style="font-size: 0.65rem; padding: 6px 10px;">
    ...
    <div class="table-card">
        ...
        <div class="table-scroll-wrapper" id="otcWrapper">
            <table class="data-table" style="min-width:1700px;">
                        ...
                        ?>
                                    ...
                                    <tr class="<?= $first ? 'otc-sale-row' : 'otc-item-row' ?>">
                                        <?php if ($first): ?>
                                            ...
                                            <td rowspan="<?= $row_count ?>" style="vertical-align:middle;border-right:1px solid var(--border-color);">
                                                <span style="font-size:0.68rem;color:var(--cyan);font-weight:800;background:var(--cyan-bg);padding:4px 8px;border-radius:5px;display:inline-block;font-family:var(--font-mono);">
                                                    <?= htmlspecialchars($otc['sale_number'] ?? 'N/A') ?>
                                                </span>
                                            </td>
                                            <td rowspan="<?= $row_count ?>" style="vertical-align:middle;border-right:1px solid var(--border-color);">
                                                <div style="font-weight:600;font-size:0.75rem;"><?= htmlspecialchars($otc['customer_name'] ?? 'Walk-in Customer') ?></div>
                                                <?php if (!empty($otc['customer_phone'])): ?>
                                                    <div style="font-size:0.62rem;color:var(--text-secondary);"><i class="fas fa-phone"></i> <?= htmlspecialchars($otc['customer_phone']) ?></div>
                                                <?php endif; ?>
                                            </td>
                                            <td rowspan="<?= $row_count ?>" style="vertical-align:middle;border-right:1px solid var(--border-color);">
                                                <span style="font-size:0.62rem;color:var(--text-secondary);font-weight:700;font-family:var(--font-mono);"><?= htmlspecialchars($bill_number) ?></span>
                                            </td>
                                        <?php endif; ?>
                                        
                                        <td class="otc-item-row">
                                            <div class="otc-item-name">
                                                <i class="fas fa-capsules"></i>
                                                <?= htmlspecialchars($pi['name']) ?>
                                            </div>
                                        </td>
                                        <td style="text-align:center;">
                                            <span class="otc-qty-badge"><?= $pi['qty'] ?></span>
                                        </td>
                                        <td class="money-cell cyan" style="font-size:0.75rem;">
                                            <span class="currency-prefix"><?= $currency ?></span><?= number_format($pi['unit_price'], 0) ?>
                                        </td>
                                        <td class="money-cell" style="font-size:0.75rem;">
                                            <span class="currency-prefix"><?= $currency ?></span><?= number_format($pi['total_price'], 0) ?>
                                        </td>
                                        ...
                                        <?php if ($first): ?>
                                            <td rowspan="<?= $row_count ?>" class="money-cell warning" style="font-size:0.75rem;vertical-align:middle;border-left:2px solid var(--border-color);">
                                                <?php if ((float)($otc['discount_amount'] ?? 0) > 0): ?>
                                                    -<span class="currency-prefix"><?= $currency ?></span><?= number_format((float)($otc['discount_amount'] ?? 0), 0) ?>
                                                <?php else: ?>
                                                    <span style="color:var(--text-secondary);">—</span>
                                                <?php endif; ?>
                                            </td>
                                            <td rowspan="<?= $row_count ?>" class="money-cell purple" style="font-size:0.75rem;vertical-align:middle;">
                                                <?php if ((float)($otc['premium_amount'] ?? 0) > 0): ?>
                                                    +<span class="currency-prefix"><?= $currency ?></span><?= number_format((float)($otc['premium_amount'] ?? 0), 0) ?>
                                                <?php else: ?>
                                                    <span style="color:var(--text-secondary);">—</span>
                                                <?php endif; ?>
                                            </td>
                                            <td rowspan="<?= $row_count ?>" class="money-cell" style="font-size:0.85rem;font-weight:900;vertical-align:middle;background:var(--cyan-bg);border-left:2px solid var(--cyan);">
                                                <span class="currency-prefix"><?= $currency ?></span><?= number_format((float)($otc['total_amount'] ?? 0), 0) ?>
                                            </td>
                                            ...
                                            <td rowspan="<?= $row_count ?>" style="vertical-align:middle;">
                                                <span style="font-size:0.7rem;color:var(--text-secondary);"><i class="fas fa-store-alt"></i> <?= htmlspecialchars($otc['branch_name'] ?? 'N/A') ?></span>
                                            </td>
                                            <td rowspan="<?= $row_count ?>" style="vertical-align:middle;">
                            ...
                            <?php else: ?>
                                <tr class="otc-sale-row">
                                    ...
                                    <td><span style="font-size:0.68rem;color:var(--cyan);font-weight:800;background:var(--cyan-bg);padding:4px 8px;border-radius:5px;font-family:var(--font-mono);"><?= htmlspecialchars($otc['sale_number'] ?? 'N/A') ?></span></td>
                                    <td><?= htmlspecialchars($otc['customer_name'] ?? 'Walk-in Customer') ?></td>
                                    ...
                                    <td style="text-align:center;"><span class="status-badge paid"><i class="fas fa-check-circle"></i> PAID</span></td>
                ...
                <tfoot>
                    <tr class="total-row">
                        <td colspan="8" style="text-align:right;font-weight:900;font-size:0.82rem;color:var(--cyan);">
                            <i class="fas fa-calculator"></i> OTC TOTAL
                        </td>
                        <td class="money-cell warning" style="font-size:0.82rem;">
                            -<span class="currency-prefix"><?= $currency ?></span><?= number_format(array_sum(array_column($otc_sales_list, 'discount_amount')), 0) ?>
                        </td>
                        <td class="money-cell purple" style="font-size:0.82rem;">
                            +<span class="currency-prefix"><?= $currency ?></span><?= number_format(array_sum(array_column($otc_sales_list, 'premium_amount')), 0) ?>
                        </td>
                        <td class="money-cell" style="font-size:0.95rem;font-weight:900;color:var(--cyan);background:var(--cyan-bg);">
                            <span class="currency-prefix"><?= $currency ?></span><?= number_format($otc_revenue, 0) ?>
                        </td>
                        <td colspan="6" style="text-align:center;font-weight:800;color:var(--cyan);font-size:0.72rem;">
                            <?= count($otc_sales_list) ?> sales
                        </td>
                    </tr>
                </tfoot>
                <?php endif; ?>
    ...
    <div class="table-card">
        <div class="table-header">
            <span class="title"><i class="fas fa-chart-pie"></i> Revenue Breakdown by Source (V52)</span>
                ...
                <tbody>
                    ...
                    <tr style="background:var(--primary-bg);">
                        <td colspan="4" style="padding:10px 16px;font-weight:800;font-size:0.75rem;color:var(--primary);text-transform:uppercase;">
                            <i class="fas fa-info-circle"></i> Patient Payments Breakdown (GROSS)
                        </td>
                    </tr>
                    <tr>
    ...
    <div class="table-card" style="border-color:<?= $net_profit >= 0 ? 'var(--success)' : 'var(--danger)' ?>;">
        <div class="table-header profit <?= $net_profit < 0 ? 'loss' : '' ?>">
            <span class="title"><i class="fas fa-chart-pie"></i> Profit Summary (V52)</span>
...
function confirmDeleteVisit(id, visitNumber) {
    ...
    document.getElementById('deleteModalText').innerHTML = 
        ...
        '<i class="fas fa-exclamation-circle"></i> This will delete ALL related bills, items, payments, prescriptions, lab tests, and procedures!</span>' +
        '<span class="modal-warning"><i class="fas fa-exclamation-circle"></i> This action cannot be undone!</span>';
...
function setDateRange(days) {
    ...
    document.getElementById('customDateFrom').value = formatDateForInput(fromDate);
...
function setMonthRange() {
    ...
    document.getElementById('customDateFrom').value = formatDateForInput(firstDay);
    document.getElementById('customDateTo').value = formatDateForInput(today);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-show custom filter if active
    <?php if ($quick_filter === 'custom'): ?>
    var section = document.getElementById('customFilterSection');
...
console.log('%c📊 Revenue Report V52 - ADMIN (FINAL)', 'font-size:18px; font-weight:bold; color:#0B5ED7;');
console.log('%c✅ V52: Prescription = GROSS pekee (bila discount, bila premium)', 'font-size:12px; color:#10B981; font-weight:bold;');
console.log('%c✅ V51: Custom Date Range Filter (date_from + date_to + presets)', 'font-size:12px; color:#10B981; font-weight:bold;');
console.log('%c✅ V50: Cards 8 TU + Wide DP Card', 'font-size:12px; color:#10B981; font-weight:bold;');
</script>
