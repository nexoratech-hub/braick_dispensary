<?php
// ================================================================
// FILE: frontend/pages/admin/audit/patient_details.php
// PATIENT DETAILS - V6 FINAL (FIXED PROCEDURES STATUS)
// ✅ V6: Procedures zinachukuliwa kutoka bill_items (status = paid)
// ✅ CSS nzuri sana (modern design)
// ✅ Kila Visit na table yake kamili
// ✅ All visit info: Patient → Visit → Vitals → Labs → Diagnosis → Meds → Procedures → Equipment → Bills
// ✅ Doctor + Reception aliyecreate
// ✅ Tarehe na saa kila mahali
...
// ================================================================
// GET BILL ITEMS - GROUPED BY BILL AND ALSO BY TYPE+VISIT
// ================================================================
$bill_items_by_bill = [];
$items_by_visit_type = []; // ✅ NEW: Group items by visit + type (for procedures, equipment, etc.)
...
try {
    ...
    foreach ($all_bill_items as $item) {
        // Group by bill_id (for bills section)
        $bid = $item['bill_id'] ?? 0;
        if (!isset($bill_items_by_bill[$bid])) $bill_items_by_bill[$bid] = [];
        $bill_items_by_bill[$bid][] = $item;
        
        // ✅ NEW: Group by visit_id + item_type (for procedures, equipment, medications sections)
        $vid = $item['visit_id'] ?? 0;
...
// ================================================================
// ✅ V6 FIX: GET PROCEDURES FROM bill_items (not from procedures table)
// This ensures STATUS matches bill_items.status = 'paid'
// ================================================================
// Procedures are now in $items_by_visit_type[$vid . '_procedure']

// ================================================================
// ✅ V6 FIX: GET EQUIPMENT FROM bill_items
// ================================================================
// Equipment are now in $items_by_visit_type[$vid . '_equipment']
...
try {
    $stmt = $db->prepare("
        ...
        WHERE r.patient_id = ? 
        ORDER BY r.referral_date DESC LIMIT 20
    ");
    $stmt->execute([$patient_id]);
...
// Count procedures from bill_items
...
/* ================================================================
   ✅ V6: VISIT SECTION - FULL CARD DESIGN
   ================================================================ */
.visits-container {
    display: flex;
    flex-direction: column;
...
/* ================================================================
   INFO GRID (for Patient Info / Visit Info)
   ================================================================ */
.info-grid {
    display: grid;
...
<main class="main-content">
    ...
    <div class="page-header">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;position:relative;z-index:1;">
            ...
            <a href="patient_edit.php?id=<?= $patient_id ?>&branch=<?= $selected_branch_id ?>" 
               class="btn-outline-light">
                <i class="fas fa-edit"></i> Edit
            </a>
            <a href="patient_pdf.php?id=<?= $patient_id ?>&branch=<?= $selected_branch_id ?>" 
               target="_blank"
    ...
    <?php if (count($visits) > 0): ?>
        <div class="visits-container">
            <?php foreach ($visits as $visit_index => $visit): 
                ...
                // ✅ V6 FIX: Get procedures from bill_items (status = paid)
            ...
            ?>
                <div class="visit-full-card">
                    ...
                    <div class="visit-full-body">
                        ...
                        <!-- ================================================
                             SECTION 7: PROCEDURES (✅ V6 FIXED - FROM bill_items)
                             ================================================ -->
                                        ...
                                        <tbody>
                                            <?php $proc_num = 1; foreach ($visit_procedures as $p): ?>
                                                <tr>
                                                    ...
                                                    <td class="money-cell" style="color:var(--teal);">
                        ...
                        <!-- ================================================
                             SECTION 8: MEDICAL EQUIPMENT (✅ V6 FIXED - FROM bill_items)
                             ================================================ -->
...
<script>
console.log('%c👤 Patient Details V6 - FIXED PROCEDURES STATUS', 'font-size:18px; font-weight:bold; color:#0B5ED7;');
console.log('%c✅ V6 FIX: Procedures zinachukuliwa kutoka bill_items (status = paid)', 'font-size:12px; color:#34D399; font-weight:bold;');
console.log('%c✅ V6 FIX: Equipment zinachukuliwa kutoka bill_items', 'font-size:12px; color:#34D399; font-weight:bold;');
console.log('%c✅ CSS nzuri sana - modern design', 'font-size:12px; color:#34D399; font-weight:bold;');
console.log('%c✅ Kila Visit na table yake kamili', 'font-size:12px; color:#34D399; font-weight:bold;');
</script>
