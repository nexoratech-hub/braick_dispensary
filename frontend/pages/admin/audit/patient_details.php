...
// PATIENT DETAILS - V4 FINAL
// ✅ Oxygen Saturation INAONEKANA (isset + check !== null)
// ✅ Blood Group kwenye Visit Information
// ✅ Vital Signs HAZIRUDIWI (tumia latest only)
// ✅ MUAC + Pain Score + Notes zinaonekana
...
try {
    ...
    // ✅ Group by visit_id - ONLY LATEST per visit (no duplicates)
    foreach ($all_vitals as $v) {
        $vid = $v['visit_id'] ?? 0;
        if (!isset($vitals_by_visit[$vid])) {
            $vitals_by_visit[$vid] = $v; // Only first (latest, since ORDER DESC)
        }
    }
} catch (Exception $e) {}
...
// ✅ Helper: Check if vital field has valid value
...
.vital-item.temp {
    ...
    background: linear-gradient(135deg, var(--bg-card) 0%, #FEF2F2 100%);
}
[data-theme="dark"] .vital-item.temp {
    background: linear-gradient(135deg, #1E293B 0%, #3A1A1A 100%);
...
[data-theme="dark"] .vital-item.pulse {
    background: linear-gradient(135deg, #1E293B 0%, #4A1E36 100%);
...
.vital-item.resp {
    ...
    background: linear-gradient(135deg, var(--bg-card) 0%, #ECFEFF 100%);
}
[data-theme="dark"] .vital-item.resp {
...
[data-theme="dark"] .vital-item.oxygen {
...
[data-theme="dark"] .vital-item.glucose {
...
[data-theme="dark"] .vital-item.weight {
...
[data-theme="dark"] .vital-item.height {
...
.vital-item.bmi {
    ...
    background: linear-gradient(135deg, var(--bg-card) 0%, #F0FDFA 100%);
}
[data-theme="dark"] .vital-item.bmi {
    background: linear-gradient(135deg, #1E293B 0%, #134E4A 100%);
...
[data-theme="dark"] .vital-item.muac {
    background: linear-gradient(135deg, #1E293B 0%, #3A2A1A 100%);
...
[data-theme="dark"] .vital-item.pain {
    background: linear-gradient(135deg, #1E293B 0%, #3A1A1A 100%);
    ...
    <?php if (count($visits) > 0): ?>
        <div class="visit-timeline">
            <?php foreach ($visits as $visit): 
                $vid = $visit['id'];
                // ✅ GET VITAL - ONLY LATEST (no duplicates)
            ...
            ?>
                <div class="visit-card">
                    ...
                    <div class="visit-card-body">
                        ...
                        <!-- 3. ✅ VITAL SIGNS - ONLY LATEST (NO DUPLICATES) -->
                        <?php if ($visit_vital !== null): 
                            // ✅ Check if has any valid vital value
                            $has_vital = false;
                        ...
                        ?>
                            ...
                            <div class="visit-section">
                                ...
                                <div class="vitals-grid">
                                    ...
                                    <!-- ✅ 5. OXYGEN SATURATION - INAONEKANA -->
...
<script>
console.log('%c👤 Patient Details V4 - FINAL', 'font-size:16px; font-weight:bold; color:#0B5ED7;');
console.log('%c✅ Oxygen Saturation: INAONEKANA', 'font-size:12px; color:#059669; font-weight:bold;');
console.log('%c✅ Blood Group: Kwenye Visit Info', 'font-size:12px; color:#DC2626; font-weight:bold;');
console.log('%c✅ Vital Signs: HAZIRUDIWI (Latest only)', 'font-size:12px; color:#34D399; font-weight:bold;');
console.log('%c✅ Allergies: Alert Header', 'font-size:12px; color:#FCD34D; font-weight:bold;');
</script>
