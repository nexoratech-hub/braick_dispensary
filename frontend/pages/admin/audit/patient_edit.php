    ...
    try {
        ...
        try {
            $db->prepare("INSERT INTO activity_logs (user_id, branch_id, action, details, ip_address, created_at) VALUES (?, ?, 'edit_patient', ?, ?, NOW())")
                ->execute([$user_id, $patient_branch_id, "Admin edited patient: {$patient['full_name']} (ID: {$patient['patient_id']})", $_SERVER['REMOTE_ADDR'] ?? 'unknown']);
        } catch (Exception $e) {}
...
[data-theme="dark"] .vital-card .vital-unit { background: #334155; }
    ...
    <div class="form-card-modern">
            ...
            <div>
                ...
                <p class="form-subtitle">
                    ...
                    <span style="color:var(--purple);font-weight:600;margin-left:6px;">
                        <i class="fas fa-crown"></i> Editing as: <?= htmlspecialchars($user_full_name) ?> (Admin)
                    </span>
                </p>
            </div>
        </div>
        ...
        <form method="POST" action="" id="editPatientForm" autocomplete="off">
            ...
            <div style="margin-top:14px;padding-top:10px;font-size:0.6rem;color:var(--text-secondary);text-align:center;border-top:1px solid var(--border-color);">
                ...
                <span style="color:var(--purple);"><i class="fas fa-crown"></i> Editing as: <strong><?= htmlspecialchars($user_full_name) ?></strong> (Admin)</span>
                <span style="margin:0 6px;">|</span>
