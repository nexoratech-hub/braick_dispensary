...
<head>
    ...
    <title>View Department - Braick Dispensary</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-blue:hover { background: #0B3D8A; transform: translateY(-2px); }
        ...
        .stat-number { font-size: 2rem; font-weight: 700; color: var(--blue-600); }
        .stat-label { font-size: 0.8rem; color: var(--gray-500); }
        .info-label { font-size: 0.75rem; color: var(--gray-400); font-weight: 500; }
        .info-value { font-size: 0.95rem; font-weight: 600; color: var(--gray-800); }
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="departments.php" class="sidebar-link active"><i class="fas fa-building"></i> Departments</a>
...
<main class="main-content">
    <div class="page-header flex flex-wrap justify-between items-center gap-3 mb-5">
        <div>
            <h1 class="page-title"><i class="fas fa-building mr-2" style="color: var(--blue-600);"></i> Department Details</h1>
            <p class="page-subtitle">View department information and employees</p>
        </div>
        <div class="flex gap-2">
            <a href="edit_department.php?id=<?= $department['id'] ?>" class="btn btn-blue btn-sm"><i class="fas fa-edit"></i> Edit</a>
            <a href="departments.php" class="btn btn-outline btn-sm"><i class="fas fa-arrow-left"></i> Back</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
        <div class="card">
            <h3 class="text-lg font-semibold text-[#0B5ED7] border-b pb-2 mb-4">
                <i class="fas fa-info-circle mr-2"></i> Department Information
            </h3>
            <div class="space-y-3">
                <div>
                    <p class="info-label">Department Name</p>
                    <p class="info-value"><?= htmlspecialchars($department['name']) ?></p>
                </div>
                <div>
                    <p class="info-label">Head of Department</p>
                    <p class="info-value"><?= htmlspecialchars($department['head_of_department'] ?? 'Not assigned') ?></p>
                </div>
                <div>
                    <p class="info-label">Description</p>
                    <p class="info-value"><?= htmlspecialchars($department['description'] ?? 'No description') ?></p>
                </div>
                <div>
                    <p class="info-label">Status</p>
                    <p class="info-value">
                        <span class="badge <?= $department['status'] === 'active' ? 'badge-success' : 'badge-danger' ?>">
                            <i class="fas fa-circle text-[5px]"></i> <?= ucfirst($department['status']) ?>
                        </span>
                    </p>
                </div>
                <div>
                    <p class="info-label">Created</p>
                    <p class="info-value"><?= date('F d, Y h:i A', strtotime($department['created_at'])) ?></p>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <h3 class="text-lg font-semibold text-[#0B5ED7] border-b pb-2 mb-4">
                <i class="fas fa-chart-bar mr-2"></i> Department Statistics
            </h3>
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="text-center p-4 bg-green-50 rounded-xl">
                    <p class="stat-number"><?= date('d/m/Y', strtotime($department['created_at'])) ?></p>
    ...
    <div class="card">
        <h3 class="text-lg font-semibold text-[#0B5ED7] border-b pb-2 mb-4">
            <i class="fas fa-users mr-2"></i> Employees in this Department
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge <?= $emp['status'] === 'active' ? 'badge-success' : 'badge-danger' ?>">
                                        <i class="fas fa-circle text-[5px]"></i> <?= ucfirst($emp['status']) ?>
                                    </span>
                                </td>
                            </tr>
                        <?php endforeach; ?>
        ...
        <?php else: ?>
            <p class="text-gray-400 text-sm py-4 text-center">No employees assigned to this department.</p>
        <?php endif; ?>
    </div>
