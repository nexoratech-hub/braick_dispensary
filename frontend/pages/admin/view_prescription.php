...
// ADMIN - PATIENT PRESCRIPTIONS (V3 - FIXED Edit Button)
// BRAICK DISPENSARY - BLUE THEME
// ✅ FIXED: Edit button inatuma visit_id pia
// ✅ Same design as patient_prescriptions.php
// ✅ Smart Delete (Pending/Confirmed/Cancelled → Return stock)
// ✅ Smart Delete (Dispensed → Keep stock)
// ================================================================
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    try {
        ...
        if (!$presc) {
            $db->rollBack();
            header('Location: view_patient_prescriptions.php?patient_id=' . $patient_id . '&branch=' . urlencode($selected_branch_id) . '&error=not_found');
            exit;
        }
        ...
        header('Location: view_patient_prescriptions.php?patient_id=' . $patient_id . '&branch=' . urlencode($selected_branch_id) . '&' . $msg);
        exit;
        
    } catch (Exception $e) {
        if ($db->inTransaction()) $db->rollBack();
        error_log("Delete prescription error: " . $e->getMessage());
        header('Location: view_patient_prescriptions.php?patient_id=' . $patient_id . '&branch=' . urlencode($selected_branch_id) . '&error=delete_failed');
        exit;
    }
}
...
if (isset($_GET['cancel']) && is_numeric($_GET['cancel'])) {
    ...
    try {
        ...
        if ($presc && $presc['status'] === 'pending') {
            ...
            header('Location: view_patient_prescriptions.php?patient_id=' . $patient_id . '&branch=' . urlencode($selected_branch_id) . '&cancelled=1');
            exit;
        } else {
            header('Location: view_patient_prescriptions.php?patient_id=' . $patient_id . '&branch=' . urlencode($selected_branch_id) . '&error=cancel_failed');
            exit;
        }
    } catch (Exception $e) {
        header('Location: view_patient_prescriptions.php?patient_id=' . $patient_id . '&branch=' . urlencode($selected_branch_id) . '&error=cancel_failed');
...
// ================================================================
// GET ALL PRESCRIPTIONS - Only with items > 0
// ================================================================
$all_prescriptions = [];
try {
    $sql = "
...
$visits_data = array_filter($visits_data, function($v) {
    return !empty($v['prescriptions']);
});
...
// ================================================================
// EDIT PRESCRIPTION URL
// ================================================================
$edit_prescription_url = '/dispensary_system/frontend/pages/admin/edit_prescription.php';
...
.btn-act.edit:hover { box-shadow: 0 4px 12px rgba(245, 158, 11, 0.5); color: white; }
...
.btn-act.cancel:hover { box-shadow: 0 4px 12px rgba(107, 114, 128, 0.5); color: white; }
...
.btn-act.delete:hover { box-shadow: 0 4px 12px rgba(220, 38, 38, 0.5); color: white; }
...
<main class="main-content">
    ...
    <div class="page-header-custom">
        <div>
            <h1 class="page-title">
                ...
                <span class="branch-tag" style="background:rgba(255,255,255,0.2);font-weight:700;text-transform:uppercase;letter-spacing:0.05em;">ADMIN</span>
            </h1>
                        ...
                        <div class="prescription-card">
                            <div class="prescription-header">
                                ...
                                <div class="presc-actions">
                                    <?php if ($is_pending): ?>
                                        <!-- ✅ FIXED: Edit button inatuma visit_id pia -->
                                        <a href="<?= $edit_prescription_url ?>?id=<?= $presc['id'] ?>&visit_id=<?= $presc['visit_id'] ?>&patient_id=<?= $patient_id ?>&branch=<?= $selected_branch_id ?>" 
                                           class="btn-act edit"
    ...
    <footer class="footer">
        <p>
            ...
            Patient Prescriptions - <?= htmlspecialchars($patient['full_name']) ?>
            <span style="margin:0 8px;">|</span>
...
console.log('%c💊 Braick - Patient Prescriptions V3 (Edit Fixed)', 'font-size:16px;font-weight:bold;color:#0B5ED7;');
console.log('%c✅ Edit button sasa inatuma: id + visit_id + patient_id + branch', 'font-size:12px;color:#F59E0B;font-weight:bold;');
console.log('%c✅ Edit inaelekeza: <?= $edit_prescription_url ?>', 'font-size:12px;color:#F59E0B;');
console.log('%c✅ Prescriptions zenye quantity = 0 HAZIONEKANI', 'font-size:12px;color:#34D399;font-weight:bold;');
console.log('%c✅ SMART DELETE inafanya kazi', 'font-size:12px;color:#0891B2;');
</script>
