...
// ✅ 8 CARDS: 4 TOP + 4 BOTTOM (ALL SAME SIZE)
// ✅ ADDED: All Discount Card
// ✅ ADDED: All Premium Card
// ✅ Uses SHARED admin sidebar
// ✅ EMBEDDED header (matches admin header exactly)
// ================================================================
...
if (isset($_GET['error'])) {
    ...
    if ($status_filter !== 'all') $redirect_url .= '&status=' . urlencode($status_filter);
...
// ================================================================
// GET FINANCIAL SUMMARY (WITH DISCOUNT & PREMIUM)
// ================================================================
function getFinancialSummary($db, $branch_id = 'all') {
    ...
    // ✅ 10. ALL DISCOUNT
    $stmt = $db->prepare("
        SELECT COALESCE(SUM(b.total_discount), 0) as total_discount
        FROM bills b
        WHERE b.total_discount > 0
        AND b.bill_number NOT LIKE 'BILL-OTC-%'
        $branch_condition
    ");
    $stmt->execute($params);
    ...
    // ✅ 11. ALL PREMIUM
    $stmt = $db->prepare("
        SELECT COALESCE(SUM(b.premium_amount), 0) as total_premium
        FROM bills b
        WHERE b.premium_amount > 0
        AND b.bill_number NOT LIKE 'BILL-OTC-%'
        $branch_condition
    ");
    $stmt->execute($params);
...
if ($selected_branch_id !== 'all' && is_numeric($selected_branch_id)) {
    ...
    if ($check_stmt->fetch()) {
        ...
        if (!empty($params_redirect)) $redirect_url .= '?' . implode('&', $params_redirect);
        header('Location: ' . $redirect_url);
        exit;
    }
}
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ================================================================
           TOP NAV - EMBEDDED HEADER (matches admin header exactly)
           ================================================================ */
        .top-nav {
            position: fixed;
            top: 0;
        ...
        .page-header-box .header-badge.premium {
        ...
        [data-theme="dark"] .card-gold { background: linear-gradient(135deg, #F59E0B, #D97706); }
        [data-theme="dark"] .card-teal { background: linear-gradient(135deg, #0D9488, #0F766E); }
        [data-theme="dark"] .card-indigo { background: linear-gradient(135deg, #4F46E5, #4338CA); }
        
        @keyframes premiumPulse {
            0%, 100% { box-shadow: 0 4px 16px rgba(217, 119, 6, 0.12); }
            50% { box-shadow: 0 4px 16px rgba(217, 119, 6, 0.4); }
        }
        ...
        .cashier-card .card-stats .stat-item .num.gold { color: #D97706; }
        ...
        @media (max-width: 480px) {
            ...
            .stat-card-8 { min-height: 110px; padding: 14px 16px; }
            ...
            <p class="page-subtitle">
                ...
                <?php if (($financial['total_premium'] ?? 0) > 0): ?>
                    <span class="header-badge premium">
                        <i class="fas fa-crown"></i> Premium: TSh <?= number_format($financial['total_premium'], 0) ?>
                    </span>
                <?php endif; ?>
            </p>
        </div>
    ...
    <div class="stats-grid-8 animate-fade-in-up" style="animation-delay:0.05s;">
        ...
        <div class="stat-card-8 card-red">
            ...
            <div class="stat-content">
                ...
                <p class="stat-sub"><i class="fas fa-file-invoice"></i> From expenses table</p>
            </div>
            <i class="fas fa-arrow-right stat-arrow"></i>
        </div>
        ...
        <!-- 7. ✅ ALL PREMIUM - GOLD (PULSE) -->
        <div class="stat-card-8 card-gold" style="animation: premiumPulse 2s ease-in-out infinite;">
...
<script>
    ...
    console.log('%c📊 8 CARDS: 4 TOP + 4 BOTTOM (ALL SAME SIZE)', 'font-size:13px; color:#0B5ED7;');
    console.log('%c   ├─ ROW 1: Revenue, Expenses, Net Profit, Prescription', 'font-size:12px; color:#0B5ED7;');
    console.log('%c   └─ ROW 2: OTC, All Discount, All Premium, Pending', 'font-size:12px; color:#D97706;');
    console.log('%c✅ Uses SHARED admin sidebar', 'font-size:13px; color:#34D399;');
    console.log('%c✅ EMBEDDED header (matches admin exactly)', 'font-size:13px; color:#34D399;');
</script>
