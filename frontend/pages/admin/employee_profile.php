...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge { padding: 3px 12px; border-radius: 20px; font-size: 0.7rem; font-weight: 600; display: inline-block; }
        .badge-success { background: #E6F7EE; color: #0AA84F; }
...
<main class="main-content">
    <div class="flex flex-wrap justify-between items-center gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-bold text-[#0B5ED7]"><i class="fas fa-user-circle mr-2"></i> Employee Profile</h1>
            <p class="text-sm text-[#64748B]">View employee details and information</p>
        </div>
        <div class="flex gap-2">
            <a href="edit_employee.php?id=<?= $employee_id ?>&branch=<?= $selected_branch_id ?>" class="btn btn-primary btn-sm">
    ...
    <div class="card mb-5">
        <div class="flex flex-col md:flex-row items-center gap-6">
            <div class="text-center">
                <div class="profile-avatar bg-[#E8F0FE] flex items-center justify-center text-[#0B5ED7] text-4xl font-bold mx-auto">
                    <?= strtoupper(substr($employee['full_name'], 0, 1)) ?>
                </div>
                <span class="badge <?= ($employee['status'] ?? 'active') === 'active' ? 'badge-success' : 'badge-danger' ?> mt-2 inline-block">
                    <i class="fas fa-circle text-[6px] mr-1"></i> <?= ucfirst($employee['status'] ?? 'Active') ?>
                </span>
            </div>
            <div class="flex-1 text-center md:text-left">
                <h2 class="text-2xl font-bold"><?= htmlspecialchars($employee['full_name']) ?></h2>
                <p class="text-[#64748B]"><?= ucfirst(htmlspecialchars($employee['role'])) ?></p>
                <div class="flex flex-wrap gap-2 mt-2 justify-center md:justify-start">
                    <span class="badge badge-info"><i class="fas fa-store-alt mr-1"></i> <?= htmlspecialchars($employee['branch_name'] ?? 'Not Assigned') ?></span>
                    <span class="badge badge-info"><i class="fas fa-user-tag mr-1"></i> ID: <?= htmlspecialchars($employee['username']) ?></span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div class="space-y-5">
            ...
            <div class="card">
                ...
                <?php else: ?>
                    <p class="text-[#94A3B8] text-sm mt-3">No roles assigned</p>
                <?php endif; ?>
            </div>

            <!-- Departments -->
            <div class="card">
                <h3 class="card-title"><i class="fas fa-building mr-2"></i> Departments</h3>
                <?php if (count($departments) > 0): ?>
                    <div class="flex flex-wrap gap-2 mt-3">
                        <?php foreach ($departments as $dept): ?>
                            <span class="badge badge-success"><?= htmlspecialchars($dept['name']) ?></span>
                        <?php endforeach; ?>
                    </div>
                <?php else: ?>
                    <p class="text-[#94A3B8] text-sm mt-3">No departments assigned</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
