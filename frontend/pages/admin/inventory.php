...
// FIXED: Edit button inside view modal works
// ================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete_medicine') {
        ...
        if ($confirmed == 1 && $id > 0) {
            try {
                $stmt = $db->prepare("SELECT id FROM medications_inventory WHERE id = ? AND branch_id = ?");
                ...
                if ($exists) {
                    ...
                    $message = "❌ Batch not found or you don't have permission to delete it.";
                    $message_type = 'error';
                }
            } catch (Exception $e) {
                $message = "❌ Error: " . $e->getMessage();
    ...
    if ($action === 'get_edit_data') {
        ...
        if ($id > 0) {
            ...
            $stmt->execute([$id, $branch_id_for_query]);
            $edit_data_ajax = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($edit_data_ajax) {
                $branches = [];
                $stmt = $db->query("SELECT id, name FROM branches WHERE status = 'active' ORDER BY name");
                $branches = $stmt->fetchAll(PDO::FETCH_ASSOC);
                
                $category_val = $edit_data_ajax['category'] ?? '';
                $is_custom_category = !in_array($category_val, $predefined_med_categories) && !empty($category_val);
                ?>
                <form method="POST" action="" class="edit-form">
                    <input type="hidden" name="action" value="edit_medicine">
                    <input type="hidden" name="id" value="<?= $edit_data_ajax['id'] ?>">
                    ...
                    <div class="form-grid">
                        <div class="full-width form-row">
                            ...
                            <input type="text" name="medication_name" class="form-control" 
                                   value="<?= htmlspecialchars($edit_data_ajax['medication_name']) ?>" required>
                        </div>
                        ...
                        <div class="form-row">
                            ...
                            <select name="unit" class="form-control">
                                <option value="pcs" <?= $edit_data_ajax['unit'] == 'pcs' ? 'selected' : '' ?>>Pieces (pcs)</option>
                                <option value="tablets" <?= $edit_data_ajax['unit'] == 'tablets' ? 'selected' : '' ?>>Tablets</option>
                                <option value="capsules" <?= $edit_data_ajax['unit'] == 'capsules' ? 'selected' : '' ?>>Capsules</option>
                                <option value="ml" <?= $edit_data_ajax['unit'] == 'ml' ? 'selected' : '' ?>>Milliliters (ml)</option>
                                <option value="mg" <?= $edit_data_ajax['unit'] == 'mg' ? 'selected' : '' ?>>Milligrams (mg)</option>
                                <option value="g" <?= $edit_data_ajax['unit'] == 'g' ? 'selected' : '' ?>>Grams (g)</option>
                                <option value="bottle" <?= $edit_data_ajax['unit'] == 'bottle' ? 'selected' : '' ?>>Bottle</option>
                                <option value="box" <?= $edit_data_ajax['unit'] == 'box' ? 'selected' : '' ?>>Box</option>
                                <option value="strip" <?= $edit_data_ajax['unit'] == 'strip' ? 'selected' : '' ?>>Strip</option>
                                <option value="vial" <?= $edit_data_ajax['unit'] == 'vial' ? 'selected' : '' ?>>Vial</option>
                                <option value="sachet" <?= $edit_data_ajax['unit'] == 'sachet' ? 'selected' : '' ?>>Sachet</option>
                            </select>
                        </div>
                        
                        <div class="form-row">
                            <label class="form-label">Quantity <span class="required">*</span></label>
                            <input type="number" name="quantity" class="form-control" value="<?= $edit_data_ajax['quantity'] ?>" min="0" required>
                        </div>
                        
                        <div class="form-row">
                            <label class="form-label">Reorder Level <span class="required">*</span></label>
                            <input type="number" name="reorder_level" class="form-control" value="<?= $edit_data_ajax['reorder_level'] ?>" min="0" required>
                        </div>
                        
                        <div class="form-row">
                            <label class="form-label">Buying Price (TSh)</label>
                            <input type="text" name="unit_cost" class="form-control money-input" value="<?= number_format($edit_data_ajax['unit_cost'] ?? 0, 0) ?>">
                        </div>
                        
                        <div class="form-row">
                            <label class="form-label">Selling Price (TSh) <span class="required">*</span></label>
                            <input type="text" name="selling_price" class="form-control money-input" value="<?= number_format($edit_data_ajax['selling_price'] ?? 0, 0) ?>" required>
                        </div>
                        
                        <div class="form-row">
                            <label class="form-label">Supplier</label>
                            <input type="text" name="supplier" class="form-control" value="<?= htmlspecialchars($edit_data_ajax['supplier'] ?? '') ?>">
                        </div>
                        
                        <div class="form-row">
                            <label class="form-label">Expiry Date</label>
                            <input type="date" name="expiry_date" class="form-control" 
                                   value="<?= $edit_data_ajax['expiry_date'] && $edit_data_ajax['expiry_date'] !== '0000-00-00' ? $edit_data_ajax['expiry_date'] : '' ?>">
                        </div>
                        
                        <div class="full-width form-row">
                            <label class="form-label">Batch Number</label>
                            <input type="text" name="batch_number" class="form-control" value="<?= htmlspecialchars($edit_data_ajax['batch_number'] ?? '') ?>">
                        </div>
                        
                        <div class="form-row">
                            <label class="form-label">Status</label>
                            <select name="status" class="form-control">
                                <option value="active" <?= $edit_data_ajax['status'] == 'active' ? 'selected' : '' ?>>Active</option>
                                <option value="inactive" <?= $edit_data_ajax['status'] == 'inactive' ? 'selected' : '' ?>>Inactive</option>
                            </select>
                        </div>
                    </div>
...
$sql = "
    SELECT COUNT(DISTINCT CONCAT(COALESCE(medication_name, ''), '-', branch_id)) as count 
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-card .stat-badge-row { display: flex; gap: 4px; margin-top: 4px; flex-wrap: wrap; position: relative; z-index: 1; }
        .stat-card .stat-badge {
            font-size: 0.5rem; font-weight: 600; padding: 1px 8px;
            border-radius: 20px; background: rgba(255,255,255,0.15);
            color: white; display: inline-flex; align-items: center; gap: 3px;
        }
        .stat-card .stat-badge.danger { background: rgba(239, 68, 68, 0.3); color: #FCA5A5; }
        .stat-card .stat-badge.warning { background: rgba(245, 158, 11, 0.3); color: #FCD34D; }
        ...
        /* Action buttons for batch table */
        .action-btn-sm {
            padding: 1px 6px;
            border-radius: 3px;
            font-size: 0.5rem;
        ...
        .delete-warning { text-align: center; padding: 16px; }
        .delete-warning i { font-size: 2.5rem; color: var(--danger); margin-bottom: 8px; }
        .delete-warning .warning-text { font-size: 0.95rem; font-weight: 600; color: var(--text-primary); }
        .delete-warning .sub-text { font-size: 0.8rem; color: var(--text-secondary); margin-top: 4px; }
...
<main class="main-content">
    ...
    <div class="branch-info-card animate-fade-in-up">
        ...
        <div class="branch-stats">
            <span class="stat-item"><i class="fas fa-pills"></i> <strong><?= $total_medicines ?></strong> entries</span>
            <span class="stat-item"><i class="fas fa-boxes"></i> <strong><?= number_format($total_quantity) ?></strong> units</span>
            <span class="stat-item"><i class="fas fa-coins"></i> <strong>TSh <?= formatMoneyShort($med_value) ?></strong></span>
            <span class="stat-item"><i class="fas fa-check-circle"></i> <strong><?= $med_in_stock ?></strong> in stock</span>
            <?php if ($med_low_stock > 0): ?>
                <span class="stat-item" style="background:rgba(245,158,11,0.3);"><i class="fas fa-exclamation-triangle"></i> <strong><?= $med_low_stock ?></strong> low</span>
            <?php endif; ?>
            <?php if ($med_out_of_stock > 0): ?>
                <span class="stat-item" style="background:rgba(239,68,68,0.3);"><i class="fas fa-times-circle"></i> <strong><?= $med_out_of_stock ?></strong> out</span>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="card animate-fade-in-up">
                                    ...
                                    <td class="col-actions">
                                        <div class="action-group">
                                            ...
                                            <button type="button" class="action-btn view" 
                                                    onclick='openViewModal(<?= $view_data_json ?>)'
                                                    title="View Batches">
                                            ...
                                            <button type="button" class="action-btn edit" 
                                                    onclick="openEditModal(<?= $first_batch_id ?>, '<?= htmlspecialchars($item['medication_name']) ?>')"
                                                    title="Edit Batch">
                                            ...
                                            <button type="button" class="action-btn delete" 
                                                    onclick="deleteBatch(<?= $first_batch_id ?>, '<?= addslashes($item['medication_name']) ?>')"
                                                    title="Delete Batch">
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <p style="color:var(--text-secondary);font-size:0.8rem;">Click "Add Medicine" to get started</p>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="modal-content">
        <div class="modal-header">
            <div class="modal-title">
                <i class="fas fa-eye"></i> Medicine Details - <span id="viewModalName"></span>
...
<div class="modal-overlay" id="editModal">
    <div class="modal-content">
        <div class="modal-header">
            <div class="modal-title">
                <i class="fas fa-edit"></i> Edit Medicine Batch - <span id="editModalName"></span>
            </div>
    ...
    <div class="modal-content">
        <div class="modal-header">
            <div class="modal-title">
                ...
                <span class="branch-name-in-title">🏥 <?= htmlspecialchars($display_branch_name) ?></span>
            </div>
        ...
        <form method="POST" action="" id="addMedicineForm">
            ...
            <div class="form-grid">
                ...
                <div class="form-row">
                    <label class="form-label">Category <span class="required">*</span></label>
                    <div class="category-input-group">
                        <select name="category" id="medCategorySelect" class="form-control" required>
                            <option value="">Select</option>
                            <?php foreach ($predefined_med_categories as $cat): ?>
...
<script>
// ================================================================
// DELETE BATCH - Uses POST request
// ================================================================
function deleteBatch(id, name) {
    if (confirm('Are you sure you want to delete this batch of "' + name + '"?\nThis action cannot be undone!')) {
...
// ================================================================
// VIEW MODAL - FIXED: Edit and Delete close view modal first
// ================================================================
function openViewModal(data) {
    ...
    if (batchIds.length > 0) {
        for (var i = 0; i < batchIds.length; i++) {
            ...
            // ================================================================
            // FIXED: Edit and Delete buttons close view modal first
            // ================================================================
            batchesHtml += `
                <tr>
                    ...
                    <td style="text-align:center;white-space:nowrap;">
                        <!-- Edit - closes view modal first -->
                        <button class="action-btn-sm edit" 
                                onclick="closeModal('viewModal');openEditModal(${batchId}, '${escapeHtml(data.name)}')" 
                                title="Edit">
                            <i class="fas fa-edit"></i>
                        </button>
                        <!-- Delete - closes view modal first -->
                        <button class="action-btn-sm delete" 
                                onclick="closeModal('viewModal');deleteBatch(${batchId}, '${escapeHtml(data.name)}')" 
                                title="Delete">
...
console.log('%c✅ Edit button inside view modal works!', 'font-size:13px; color:#34D399; font-weight:bold;');
console.log('%c✅ Delete button inside view modal works!', 'font-size:13px; color:#34D399;');
console.log('%c✅ Category is required for Add and Edit', 'font-size:13px; color:#34D399;');
</script>
